...
static std::unordered_map<std::string, THDChannelType> name2channel_type = {
    {"mpi", THDChannelMPI}, {"tcp", THDChannelTCP}, {"gloo", THDChannelGloo},
};
...
#define ASSERT_NOT_NULL(ptr)                                                   \
  ...
  if (!THDPDoubleTensor_postInit(torch_module))
    return false;
  if (!THDPFloatTensor_postInit(torch_module))
    return false;
  if (!THDPZDoubleTensor_postInit(torch_module))
    return false;
  if (!THDPZFloatTensor_postInit(torch_module))
    return false;
  // if (!THDPHalfTensor_postInit(torch_module)) return false;
  if (!THDPLongTensor_postInit(torch_module))
    return false;
  if (!THDPIntTensor_postInit(torch_module))
    return false;
  if (!THDPShortTensor_postInit(torch_module))
    return false;
  if (!THDPCharTensor_postInit(torch_module))
    return false;
  if (!THDPByteTensor_postInit(torch_module))
    return false;

  ASSERT_NOT_NULL(THDPDoubleStorageClass = PyObject_GetAttrString(
                      torch_module, (char *)"DoubleStorage"));
  ASSERT_NOT_NULL(THDPFloatStorageClass = PyObject_GetAttrString(
                      torch_module, (char *)"FloatStorage"));
  ASSERT_NOT_NULL(THDPZDoubleStorageClass = PyObject_GetAttrString(
                      torch_module, (char *)"ZDoubleStorage"));
  ASSERT_NOT_NULL(THDPZFloatStorageClass = PyObject_GetAttrString(
                      torch_module, (char *)"ZFloatStorage"));
  // ASSERT_NOT_NULL(THDPHalfStorageClass   =
  // PyObject_GetAttrString(torch_module,(char*)"HalfStorage"));
  ASSERT_NOT_NULL(THDPLongStorageClass = PyObject_GetAttrString(
                      torch_module, (char *)"LongStorage"));
  ASSERT_NOT_NULL(THDPIntStorageClass = PyObject_GetAttrString(
                      torch_module, (char *)"IntStorage"));
  ASSERT_NOT_NULL(THDPShortStorageClass = PyObject_GetAttrString(
                      torch_module, (char *)"ShortStorage"));
  ASSERT_NOT_NULL(THDPCharStorageClass = PyObject_GetAttrString(
                      torch_module, (char *)"CharStorage"));
  ASSERT_NOT_NULL(THDPByteStorageClass = PyObject_GetAttrString(
...
#define INIT_STATELESS(type)                                                   \
  stateless = PyObject_CallFunctionObjArgs(                                    \
      (PyObject *)&TH_CONCAT_3(THDP, type, TensorStatelessType), NULL);        \
  if (!stateless) {                                                            \
    return false;                                                              \
  }                                                                            \
  if (PyObject_SetAttrString(TH_CONCAT_3(THDP, type, TensorClass),             \
...
PyObject *THDPModule_initProcessGroup(PyObject *_unused, PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 5 ||
      !THPUtils_checkString(PyTuple_GET_ITEM(args, 0)) ||
    ...
    THPUtils_invalidArguments(args, NULL, "init_process_group", 1,
                              "(string backend, string init_method, int "
                              "world_size, string group_name, int rank)");
    return NULL;
  }
...
PyObject *THDPModule_initMasterWorker(PyObject *_unused, PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 5 ||
      ...
      !THPUtils_checkString(PyTuple_GET_ITEM(args, 3)) ||
      !THPUtils_checkLong(PyTuple_GET_ITEM(args, 4))) {
    THPUtils_invalidArguments(args, NULL, "init_master_worker", 1,
                              "(string backend, string init_method, int "
                              "world_size, string group_name, int rank)");
    return NULL;
  }
...
#define REGISTER_THC_DESCRIPTOR(TYPE)                                          \
  if (type == THCP##TYPE##Class)                                               \
    return THDTensorDescriptor_newFromTHCuda##TYPE(                            \
        (THCuda##TYPE *)(((torch::THPVoidTensor *)obj)->cdata));
  REGISTER_THC_DESCRIPTOR(DoubleTensor);
  if (type == THCPFloatTensorClass)
    return THDTensorDescriptor_newFromTHCudaFloatTensor(
        (THCudaTensor *)(((torch::THPVoidTensor *)obj)->cdata));
...
#undef REGISTER_THC_DESCRIPTOR
  ...
  throw std::runtime_error(
      std::string("don't know how to create a THDTensorDesciptor for "
                  "type ") +
...
PyObject *THDPModule_isend(PyObject *_unused, PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 2 ||
      !THPModule_isTensor(PyTuple_GET_ITEM(args, 0)) ||
      !THPUtils_checkLong(PyTuple_GET_ITEM(args, 1))) {
    THPUtils_invalidArguments(args, NULL, "isend", 1,
...
PyObject *THDPModule_irecv(PyObject *_unused, PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 2 ||
      !THPModule_isTensor(PyTuple_GET_ITEM(args, 0)) ||
      !THPUtils_checkLong(PyTuple_GET_ITEM(args, 1))) {
    THPUtils_invalidArguments(args, NULL, "irecv", 1,
...
PyObject *THDPModule_send(PyObject *_unused, PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 2 ||
      !THPModule_isTensor(PyTuple_GET_ITEM(args, 0)) ||
      !THPUtils_checkLong(PyTuple_GET_ITEM(args, 1))) {
    THPUtils_invalidArguments(args, NULL, "send", 1,
                              "(tensor input, int dst_rank)");
    return NULL;
  }
...
PyObject *THDPModule_recv(PyObject *_unused, PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 2 ||
      !THPModule_isTensor(PyTuple_GET_ITEM(args, 0)) ||
      !THPUtils_checkLong(PyTuple_GET_ITEM(args, 1))) {
    THPUtils_invalidArguments(args, NULL, "recv", 1,
                              "(tensor output, int src_rank)");
    return NULL;
  }
...
PyObject *THDPModule_allReduce(PyObject *_unused, PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 3 ||
      !THPModule_isTensor(PyTuple_GET_ITEM(args, 0))) {
    THPUtils_invalidArguments(args, NULL, "all_reduce", 1,
...
PyObject *THDPModule_allGather(PyObject *_unused, PyObject *args) {
  ...
  for (std::size_t i = 0; i < length; ++i) {
    ...
    descriptors.push_back(THDPTensorDesc(
...
PyObject *THDPModule_requestIsCompleted(PyObject *_unused, PyObject *_req) {
  ...
  if (!THPWrapper_check(_req)) {
    THPUtils_invalidArguments(_req, NULL, "requestIsCompleted", 1,
                              "(request req)");
    return NULL;
  }
...
PyObject *THDPModule_initExtension(PyObject *_unused, PyObject *args) {
  ...
  THPUtils_assert(PyBool_Check(is_master_worker_obj),
...
#undef REGISTER_GROUP
  ...
  if (is_master_worker) {
    ...
    THPUtils_assert(module,
                    "class loader couldn't access torch.distributed module");
    PyObject *module_dict = PyModule_GetDict(module);
    if (!THDPModule_loadClasses(module_dict))
      return NULL;
    if (!THDPModule_assignStateless(module_dict))
      return NULL;
  }
  Py_RETURN_TRUE;
}
...
static struct PyMethodDef _THDPModule_methods[] = {
    {"_dist_init_extension", (PyCFunction)THDPModule_initExtension,
     METH_VARARGS, NULL},
    {"_dist_init_process_group", (PyCFunction)THDPModule_initProcessGroup,
     METH_VARARGS, NULL},
    {"_dist_init_master_worker", (PyCFunction)THDPModule_initMasterWorker,
     METH_VARARGS, NULL},
#ifdef WITH_CUDA
    ...
    {"_dist_get_num_processes", (PyCFunction)THDPModule_getNumProcesses,
     METH_NOARGS, NULL},
    ...
    {"_dist_request_is_completed", (PyCFunction)THDPModule_requestIsCompleted,
     METH_O, NULL},
    {"_dist_request_wait", (PyCFunction)THDPModule_requestWait, METH_O, NULL},
    {NULL}};

PyMethodDef *THDPModule_methods() { return _THDPModule_methods; }
