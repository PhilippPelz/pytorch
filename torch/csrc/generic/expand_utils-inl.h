// We need separate THC_STATE from LIBRARY_STATE because this is called for copy
// functions.
// Copy functions have a THCState if either the source or destination are GPU
// types,
// but expand functions don't have this property.  So, if we have a source CPU
// type
// and destination GPU type, we will have a THCState but shouldn't pass it to
// the expand
// function.

#define IMPLEMENT_NEWFOREXPAND(TYPEC, THC_STATE_NOARGS)                        \
  template <> TH##TYPEC##Tensor *newForExpand(LIBRARY_STATE_TYPE_NOARGS) {     \
...
IMPLEMENT_NEWFOREXPAND(ZFloat, )
IMPLEMENT_NEWFOREXPAND(ZDouble, )
...
IMPLEMENT_NEWFOREXPAND(CudaZDouble, LIBRARY_STATE_NOARGS)
IMPLEMENT_NEWFOREXPAND(CudaZFloat, LIBRARY_STATE_NOARGS)
...
#define IMPLEMENT_EXPAND(TYPEC, THC_STATE)                                     \
  template <>                                                                  \
  void expand(LIBRARY_STATE_TYPE TH##TYPEC##Tensor *r,                         \
              TH##TYPEC##Tensor *tensor, THLongStorage *sizes) {               \
...
IMPLEMENT_EXPAND(ZFloat, )
IMPLEMENT_EXPAND(ZDouble, )
...
IMPLEMENT_EXPAND(CudaZDouble, LIBRARY_STATE)
IMPLEMENT_EXPAND(CudaZFloat, LIBRARY_STATE)
#ifdef CUDA_HALF_TENSOR
