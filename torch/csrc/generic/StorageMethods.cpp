...
static PyObject *THPStorage_(copy_)(PyObject *self, PyObject *args,
                                    PyObject *kwargs) {
  HANDLE_TH_ERRORS
...
static PyObject *THPStorage_(fill_)(THPStorage *self, PyObject *number_arg) {
  ...
  THPUtils_assert(THPUtils_(checkReal)(number_arg), "fill_ expects %s, "
                  ...
                  THPUtils_typename(number_arg));
  THStorage_(fill)(LIBRARY_STATE self->cdata,
...
static PyObject *THPStorage_(fromBuffer)(PyObject *_unused, PyObject *args,
                                         PyObject *keywds) {
  HANDLE_TH_ERRORS
  PyObject *obj = NULL;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, argtypes, kwlist, &obj,
                                   &byte_order_str, &count, &offset)) {
    return NULL;
  }
  ...
  if (count < 0) {
    if ((buffer.len - offset) % sizeof(real) != 0) {
      PyErr_Format(PyExc_ValueError, "buffer size (%ld) must be a multiple "
                                     "of element size (%ld)",
                   (long)buffer.len, (long)sizeof(real));
      PyBuffer_Release(&buffer);
      return NULL;
    }
  ...
  if (offset + (count * (Py_ssize_t)sizeof(real)) > buffer.len) {
    PyErr_Format(PyExc_ValueError, "buffer has only %ld elements after offset "
                                   "%ld, but specified a size of %ld",
                 (long)(buffer.len - offset), (long)offset, (long)count);
    PyBuffer_Release(&buffer);
    return NULL;
  }
  ...
  THP_decodeInt64Buffer((int64_t *)storage->data, src + offset, byte_order,
                        count);
  ...
  THP_decodeZFloatBuffer(storage->data, src + offset, byte_order, count);
#elif defined(TH_REAL_IS_ZDOUBLE)
  THP_decodeZDoubleBuffer(storage->data, src + offset, byte_order, count);
#else
#error "Unknown type"
#endif
...
static PyObject *THPStorage_(fromFile)(PyObject *_unused, PyObject *args,
                                       PyObject *keywds) {
  HANDLE_TH_ERRORS
  const char *filename;
  Py_ssize_t size = 0;
  ...
  if (!PyArg_ParseTupleAndKeywords(args, keywds, "s|in", kwlist, &filename,
                                   &shared, &size)) {
    return NULL;
  }
  if (shared)
  ...
  THStorage *storage =
      THStorage_(newWithMapping)(LIBRARY_STATE filename, size, shared);
  return (PyObject *)THPStorage_(New)(storage);
  END_HANDLE_TH_ERRORS
}
#ifndef THD_GENERIC_FILE
...
PyObject *THPStorage_(_setCdata)(THPStorage *self, PyObject *new_cdata) {
  HANDLE_TH_ERRORS
  THPUtils_assert(THPUtils_checkLong(new_cdata),
...
PyObject *THPStorage_(_rootStorage)(THPStorage *self) {
  ...
  PyObject *result =
      Py_BuildValue("(NN)", storage.get(), PyLong_FromLong(offset));
  storage.release();
  return result;
  END_HANDLE_TH_ERRORS
}
...
static PyMethodDef THPStorage_(methods)[] = {
    {"copy_", (PyCFunction)THPStorage_(copy_), METH_VARARGS | METH_KEYWORDS,
     NULL},
    ...
    {"_new_with_file", (PyCFunction)THPStorage_(newWithFile),
     METH_O | METH_STATIC, NULL},
    {"_set_from_file", (PyCFunction)THPStorage_(setFromFile), METH_VARARGS,
     NULL},
#endif // !defined(THD_GENERIC_FILE)
#if !defined(THC_GENERIC_FILE) && !defined(THD_GENERIC_FILE)
    {"from_buffer", (PyCFunction)THPStorage_(fromBuffer),
     METH_VARARGS | METH_KEYWORDS | METH_STATIC, NULL},
#endif
    {"from_file", (PyCFunction)THPStorage_(fromFile),
     METH_VARARGS | METH_KEYWORDS | METH_STATIC, NULL},
#ifdef THC_GENERIC_FILE
...
#ifndef THD_GENERIC_FILE
    {"_root_storage", (PyCFunction)THPStorage_(_rootStorage), METH_NOARGS,
     NULL},
#endif
    {NULL}};
