...
THTensor *THPTensor_(fromNumpy)(PyObject *numpy_array) {
  ...
  if (PyArray_SIZE(array) != 0) {
    ...
    THStoragePtr storage = THStorage_(newWithDataAndAllocator)(
        (real *)PyArray_DATA(array), storage_size, &THNumpyArrayAllocator,
...
static bool THPTensor_(_index)(THPTensor *self, PyObject *index,
  ...
  if (PyTuple_Check(index)) {
    ...
    for (int dim = 0; dim < num_index_dim; dim++) {
      if (dim == ellipsis_idx) {
        ...
        if (tresult)
          indexed_dim = tresult->nDimension - (num_index_dim - dim - 1);
        continue;
      }
      PyObject *dimidx = PyTuple_GET_ITEM(index, dim);
      valid = THPTensor_(_indexOnce)(dimidx, indexed_dim, tresult, sresult,
                                     storage_offset);
      if (!valid) {
        tresult = NULL;
  ...
  PyErr_Format(PyExc_TypeError,
               "indexing a tensor with an object of type %s. "
...
#else
               "torch.cuda.LongTensor or torch.cuda.ByteTensor as the only "
               "argument.",
#endif
...
static PyObject *THPTensor_(getValue)(THPTensor *self, PyObject *index) {
  ...
  THPUtils_setError(
      "An unknown error has occured when indexing a tensor "
