...
#endif
#ifdef TH_REAL_IS_ZDOUBLE
#define NUMPY_TYPE_ENUM NPY_CDOUBLE
#endif
#ifdef TH_REAL_IS_ZFLOAT
#define NUMPY_TYPE_ENUM NPY_CFLOAT
#endif
...
#define COPY_FROM_ARRAY_CUDA_COMPLEX_FLOAT(ELTYPE, ARRAY, STORAGE, SIZE)       \
  {                                                                            \
    ELTYPE *arrdata = (ELTYPE *)PyArray_DATA(ARRAY);                           \
    std::unique_ptr<load_real> data_guard(new load_real[SIZE]);                \
    load_real *data = data_guard.get();                                        \
    for (size_t i = 0; i < SIZE; i++) {                                        \
      data[i] = toCcx(arrdata[i]);                                             \
    }                                                                          \
    THHostStorage *cpu_storage = THHostStorage_(newWithData)(                  \
        reinterpret_cast<cx *>(data_guard.get()), SIZE);                       \
    cpu_storage->flag &= ~TH_STORAGE_FREEMEM;                                  \
    THCStorage_(copyCPU)(LIBRARY_STATE STORAGE, cpu_storage);                  \
    THHostStorage_(free)(cpu_storage);                                         \
  }

#define COPY_FROM_ARRAY_CUDA_COMPLEX_DOUBLE(ELTYPE, ARRAY, STORAGE, SIZE)      \
  {                                                                            \
    ELTYPE *arrdata = (ELTYPE *)PyArray_DATA(ARRAY);                           \
    std::unique_ptr<load_real> data_guard(new load_real[SIZE]);                \
    load_real *data = data_guard.get();                                        \
    for (size_t i = 0; i < SIZE; i++) {                                        \
      data[i] = toZcx(arrdata[i]);                                             \
    }                                                                          \
    THHostStorage *cpu_storage = THHostStorage_(newWithData)(                  \
        reinterpret_cast<zx *>(data_guard.get()), SIZE);                       \
    cpu_storage->flag &= ~TH_STORAGE_FREEMEM;                                  \
    THCStorage_(copyCPU)(LIBRARY_STATE STORAGE, cpu_storage);                  \
    THHostStorage_(free)(cpu_storage);                                         \
  }
...
#define COPY_FROM_ARRAY COPY_FROM_ARRAY_CUDA_COMPLEX_FLOAT
#elif defined(THC_REAL_IS_ZDOUBLE)
#define COPY_FROM_ARRAY COPY_FROM_ARRAY_CUDA_COMPLEX_DOUBLE
#else
...
THTensor *THPTensor_(fromNumpy)(PyObject *numpy_array) {
  ...
  if (PyArray_SIZE(array) != 0) {
    ...
    for (int i = 0; i < ndim; ++i) {
      ...
      if (strides_data[i] < 0) {
        THPUtils_setError(
            "some of the strides of a given numpy array are "
    ...
    if (PyArray_TYPE(array) == NUMPY_TYPE_ENUM) {
      THStoragePtr storage(THStorage_(newWithDataAndAllocator)(
          LIBRARY_STATE(real *) PyArray_DATA(array), storage_size,
          // See Note [Numpy memory management]
          &THNumpyArrayAllocator, new NumpyArrayAllocator(numpy_array)));
      result =
          THTensor_(newWithStorage)(LIBRARY_STATE storage, 0, sizes, strides);
    } else
#endif
    {
      ...
      case NPY_DOUBLE:
        COPY_FROM_ARRAY(double, array, storage, storage_size);
        break;
      case NPY_FLOAT:
        COPY_FROM_ARRAY(float, array, storage, storage_size);
        break;
#if defined(THC_REAL_IS_ZDOUBLE)
      case NPY_CDOUBLE:
        COPY_FROM_ARRAY(double _Complex, array, storage, storage_size);
        break;
#endif
#if defined(THC_REAL_IS_ZFLOAT)
      case NPY_CFLOAT:
        COPY_FROM_ARRAY(float _Complex, array, storage, storage_size);
        break;
#endif
      case NPY_INT64:
        COPY_FROM_ARRAY(int64_t, array, storage, storage_size);
        break;
      case NPY_INT32:
        COPY_FROM_ARRAY(int32_t, array, storage, storage_size);
        break;
      case NPY_INT16:
        COPY_FROM_ARRAY(int16_t, array, storage, storage_size);
        break;
      case NPY_UINT8:
        COPY_FROM_ARRAY(uint8_t, array, storage, storage_size);
        break;
      }
      result =
          THTensor_(newWithStorage)(LIBRARY_STATE storage, 0, sizes, strides);
...
static PyObject *THPTensor_(pynew)(PyTypeObject *type, PyObject *args,
                                   PyObject *kwargs) {
  HANDLE_TH_ERRORS
  ...
  if (kwargs != NULL) {
    ...
    if (device_id == Py_None) {
      ...
      THPUtils_assert(THPUtils_checkLong(device_id),
                      "device argument "
                      " has to be an int, but got %s",
                      THPUtils_typename(device_id));
...
// This is an internal option, so we don't want to advertise it.
    ...
    THPUtils_assert(num_kwargs == 0,
                    THPTensorStr " constructor only "
                                 "accepts a 'device' keyword argument")
#else
    THPUtils_assert(num_kwargs == 0,
                    THPTensorStr " constructor doesn't "
  ...
  if (num_args == 1 && THPStorage_(Check)(first_arg)) {
    ...
    self->cdata = THTensor_(newWithStorage1d)(LIBRARY_STATE storage, 0,
                                              storage->size, -1);
    return (PyObject *)self.release();
  }
  ...
  if (num_args == 1 && PyArray_Check(first_arg)) {
    THPObjectPtr numpy_array(PyArray_FromArray((PyArrayObject *)first_arg,
                                               nullptr, NPY_ARRAY_BEHAVED));
  ...
  if (num_args == 1 && PySequence_Check(first_arg)) {
    ...
    for (auto size : sizes)
      *sizes_data++ = size;
    THTensorPtr tensor(
        THTensor_(newWithSize)(LIBRARY_STATE sizes_storage, NULL));
    ...
    while (true) {
      ...
      try {
        ...
        THPUtils_setError("tried to construct a tensor from a %s%s sequence, "
                          ...
                          THPUtils_typename(item.get()), index.c_str());
        return NULL;
      }
#ifdef THC_GENERIC_FILE
#ifdef THC_REAL_IS_HALF
      THFloatStorage *cpu_storage =
          THFloatStorage_newWithData(data_guard.get(), numel);
      cpu_storage->flag &= ~TH_STORAGE_FREEMEM;
      THCudaHalfStorage_copyFloat(LIBRARY_STATE tensor->storage, cpu_storage);
      THFloatStorage_free(cpu_storage);
#elif defined(THC_REAL_IS_ZFLOAT)
      THHostStorage *cpu_storage = THHostStorage_(newWithData)(
          reinterpret_cast<cx *>(data_guard.get()), numel);
      cpu_storage->flag &= ~TH_STORAGE_FREEMEM;
      THCStorage_(copyCPU)(LIBRARY_STATE tensor->storage, cpu_storage);
      THHostStorage_(free)(cpu_storage);
#elif defined(THC_REAL_IS_ZDOUBLE)
      THHostStorage *cpu_storage = THHostStorage_(newWithData)(
          reinterpret_cast<zx *>(data_guard.get()), numel);
      cpu_storage->flag &= ~TH_STORAGE_FREEMEM;
      THCStorage_(copyCPU)(LIBRARY_STATE tensor->storage, cpu_storage);
      THHostStorage_(free)(cpu_storage);
#else
...
#undef load_real
      ...
      for (int i = last_updated_dim + 1; i < ndims; i++) {
        ...
        if (length < 0) {
          ...
          THPUtils_setError("could not obtain a length of %s at index %s",
                            THPUtils_typename(sequences[i].get()),
                            index_str.c_str());
          return NULL;
        }
  ...
  THPUtils_invalidArguments(
      args, kwargs, THPTensorStr " constructor", 6, "no arguments", "(int ...)",
      "(" THPTensorStr " viewed_tensor)", "(torch.Size size)",
      "(" THPStorageStr " data)", "(Sequence data)");
  return NULL;
...
static bool THPTensor_(_indexOnce)(PyObject *index, int &indexed_dim,
                                   THTensorPtr &tresult, THStorage *&sresult,
                                   long &storage_offset) {
#ifdef WITH_NUMPY
  ...
  if (IS_SCALAR(index)) {
    ...
    if (idx < 0 || idx >= dimsize) {
      PyErr_Format(PyExc_IndexError, "index %lld is out of range for dimension "
                                     "%lld (of size %lld)",
                   (long long)idx, (long long)indexed_dim, (long long)dimsize);
      throw python_error();
    }
    ...
    // _indexOnce will never be called with tresult == NULL, except for a None
    // index
    // e.g. x = torch.Tensor(5); y = x[5, None]
    if (!tresult) {
      tresult = THTensor_(newWithStorage1d)(LIBRARY_STATE sresult,
                                            storage_offset, 1, 1);
      sresult = NULL;
    } else {
    ...
    if (!THPUtils_parseSlice(
            index, THTensor_(size)(LIBRARY_STATE tresult.get(), indexed_dim),
...
static bool
    THPTensor_(_checkSingleSequenceTriggersAdvancedIndexing)(PyObject *arg) {
...
static bool THPTensor_(_checkBasicIntegerArrayIndexing)(THPTensor *indexed,
                                                        PyObject *arg) {
...
static bool THPTensor_(_checkAdvancedIndexing)(THPTensor *indexed,
                                               PyObject *arg) {
  // Currently we only support two forms of advanced indexing:
  //
  // 1. Indexing with a single non-tuple sequence, not nested within a
  // sequence,
  ...
  // 3. Combining Advanced Indexing with ":", or "..." , with the limitation
  // that
  ...
  if (THPTensor_(_checkSingleSequenceTriggersAdvancedIndexing)(arg))
    return true;

  // Verification, Step #2 -- ndim sequencers
  if (THPTensor_(_checkBasicIntegerArrayIndexing)(indexed, arg))
    return true;
  ...
  if (PySequence_Check(arg) && PySequence_Size(arg) <= ndim + 1) {
    ...
    // Note that we can have ndim + 1 Tensors in the case where we have an
    // ellipsis,
    // because Python semantics allow it to be "thrown away" so to speak. If
    // this is
    // the case, we have to shift the dimension we are considering (in the
    // indexed
    // tensor) by -1 afer encountering the Ellipsis when accessing properties
    // of
    // the indexed Tensor
    ...
    for (Py_ssize_t i = 0; i < PySequence_Fast_GET_SIZE(fast.get()); ++i) {
      ...
      if (PySlice_Check(item)) {
        long dimSize = THTensor_(size)(LIBRARY_STATE indexed->cdata,
                                       correspondingTensorDim);
  ...
  // Full NumPy advanced indexing requirements are coded up below. To fully
  // support
  // such indexing will require changes to the actual indexing logic, so we
  // will
  ...
  if (PyArray_Check(arg) && (PyArray_TYPE((PyArrayObject*)arg) == NPY_INT64 ||
PyArray_TYPE((PyArrayObject*)arg) == NPY_BOOL)) return true;
#endif

  // Case 3: arg is a tuple containing at least one sequence object, ndarray,
or
LongTensor
...
#ifdef WITH_NUMPY
      if (PyArray_Check(item) && (PyArray_TYPE((PyArrayObject*)item) ==
NPY_INT64 || PyArray_TYPE((PyArrayObject*)item) == NPY_BOOL)) return true;
#endif
...
static PyObject *THPTensor_(checkAdvancedIndexing)(THPTensor *self,
                                                   PyObject *arg) {
...
static bool THPTensor_(_convertToTensorIndexers)(
    PyObject *index, THTensorPtr &indexed, Py_ssize_t &sequenceLength,
  ...
  // another sequence type, we attempt to convert it to a LongTensor, and
  // record
  // its position.
  //
  // Next, once we have all of the indexing Tensors, we attempt to broadcast
  // them.
  // If they can be broadcasted, we store each of the broadcasted Tensors in
  // the
  // output map, with the dimension of the original tensor as the key.

  // indexingDims stores the indices containing an advanced index sequence,
  // and
  // indexers
  // stores the corresponding indexing object, such that the indexer at
  // indexers[i] is
  // associated with the dm at indexingDims[i]. This is pre-broadcast. Because
  // we rely
  // upon the THPIndexTensor constructor to handle sequence -> tensor
  // conversions, we
  ...
  if (THPTensor_(_checkSingleSequenceTriggersAdvancedIndexing)(index)) {
    ...
    if (!indexer) {
      PyErr_Format(PyExc_IndexError, "When performing advanced indexing the "
                                     "indexing objects must be LongTensors or "
    ...
    for (Py_ssize_t i = 0; i < sequenceLength; ++i) {
      ...
      if (Py_TYPE(item) == &PyEllipsis_Type) {
        if (sequenceLength !=
            (THTensor_(nDimension)(LIBRARY_STATE indexed) + 1)) {
          ellipsisOffset =
              THTensor_(nDimension)(LIBRARY_STATE indexed) - sequenceLength;
        } else {
          ellipsisOffset = -1;
        }
        continue;
      ...
      if (!PySlice_Check(item)) {
        ...
        THPIndexTensor *indexer =
            (THPIndexTensor *)PyObject_CallFunctionObjArgs(THPIndexTensorClass,
                                                           obj, NULL);
        if (!indexer) {
          PyErr_Format(PyExc_IndexError,
                       "When performing advanced indexing the indexing objects "
                       "must be LongTensors or "
                       "convertible to LongTensors. The indexing object at "
                       "position %zd is of type %s "
                       "and cannot be converted",
                       i, THPUtils_typename(obj));
  ...
  try {
    THIndexTensor_(expandNd)(LIBRARY_STATE maybeBroadcasted.data(),
                             candidates.data(), maybeBroadcasted.size());

    // Broadcast succeeded, place Broadcasted Tensors into output map by the
    // index at
...
// will trigger a device-side assert
...
#if !defined(THC_GENERIC_FILE)
    ptrdiff_t nElement = THIndexTensor_(nElement)(
        LIBRARY_STATE broadcasted.begin()->second.get());
    ...
    for (auto &dimBroadcast : broadcasted) {
      ...
      THPPointer<THIndexTensor> contig(THIndexTensor_(newContiguous)(
          LIBRARY_STATE dimBroadcast.second.get()));

      // View as 1D + get1D makes me sad :(
      THPPointer<THIndexTensor> flat(
          THIndexTensor_(newView)(LIBRARY_STATE contig.get(), viewer));
      for (ptrdiff_t i = 0;
           i < THIndexTensor_(nElement)(LIBRARY_STATE flat.get()); ++i) {
        long indexAtDim = THTensor_fastGet1d(flat.get(), i);
        if (indexAtDim >= sizeAtDim) {
          PyErr_Format(PyExc_IndexError,
                       "index %lld from broadcast indexer is out of range "
                       "for dimension %lld (of size %lld)",
                       (long long)indexAtDim, (long long)dim,
                       (long long)sizeAtDim);
    ...
    // Broadcasted failed, cleanup and return error. I'm not sure if there is
    // a
    // better
    ...
    PyErr_Format(PyExc_IndexError,
                 "The advanced indexing objects could not be broadcast");
...
static inline long THPTensor_(_indexToOffset)(
    ...
    std::unordered_map<Py_ssize_t, THPPointer<THIndexTensor>> &broadcasted,
    ptrdiff_t index) {
  // We need to translate an "index" into a linear offset within the Tensor
  // indexed.
  // We will perform the normal mod/divide loop, except in the case of an
  // advance indexed
  // dimension, we need to take special care to utilize the size and subset of
  // indices
  // specified by the Tensor at the advanced indexed dimension. We hereafter
  // refer to
  // this as the "broadcast" dimension, although in the case of a single
  // indexer, the
  // broadcast op is pretty much a no-op.
  //
  // For example, suppose we have a three-dimensional Tensor x of shape (5,
  // 10,
  // 15),
  ...
  //
  // So we have selected the index (0, 2, 2). Now for the strides calculation.
  // For the
  // non-broadcast dimensions, we simply do the index * the stride. But for
  // the
  // broadcast
  // dimension we need to get the corresponding subset index (i.e., pick from
  // (2, 4, 5))
  ...
  //
  // The other complication occurs when we have more than one advanced
  // indexer.
  // Consider
  // the case:
  //
  ...
  // x[:, [0, 1], [2, 3], :]
  //
  // Because the advanced indexers are broadcast and iterated as one, we need
  // to
  // apply
  // the same index in each of the advanced indexing dimensions. When we reach
  // an advanced
  // indexing element, we look to see if the next dimension we will consider
  // is
  // also part
  ...
  //
  // Special care needs to be taken to handle advanced indexers at the
  // beginning, end.

  long offset = 0;
  for (long i = THTensor_(nDimension)(LIBRARY_STATE indexed) - 1; i >= 0; --i) {
    // Get size at dimension i, its the size of the indexed Tensor at that
    // dimension if its
    // not an advanced indexing dimension, otherwise its the size of the
    // broadcast Tensor
    ...
    if (broadcast != broadcasted.end()) {
      ...
      indexAtDim =
          THTensor_fastGet1d(broadcast->second.get(), index % sizeAtDim);
  ...
  // size at dim is a bad name, because its really the number of elements in
  // the
...
static THIndexTensor *THPTensor_(_calculateLinearIndices)(
    THTensorPtr &indexed, Py_ssize_t sequenceLength,
    std::unordered_map<Py_ssize_t, THPPointer<THIndexTensor>> &broadcasted) {

  // Get the number of indices to generate - this is the product of the size
  // at
  // each dimension,
  // that is not part of the advanced indexing, multiplied by the nElement of
  ...
  //                 --> multiply by nElement of broadcast Tensor, nElement =
  //                 2
  //                 --> total_size = 50

  // TODO: should this be 1? what if there are no things to index? ????
  ptrdiff_t indexingElements =
      THIndexTensor_(nElement)(LIBRARY_STATE broadcasted.begin()->second.get());
  for (Py_ssize_t i = 0; i < THTensor_(nDimension)(LIBRARY_STATE indexed.get());
       ++i) {
    indexingElements *= broadcasted.find(i) != broadcasted.end()
                            ? 1
                            : THTensor_(size)(LIBRARY_STATE indexed.get(), i);
  }

  // The broadcasted advanced indexing tensor might not be one-dimensional,
  // but
  // we are
  // generating a vector of indices, so we need to view the indexer as 1D
  // prior
  // to getting
  // the value for the particular dimension.
  std::unordered_map<Py_ssize_t, THPPointer<THIndexTensor>>
      flattenedBroadcasters;
  THLongStorage *indexerSize = THLongStorage_newWithSize(1);

  // All broadcast Tensors have the same number of elements
  ptrdiff_t dimIndexingElements =
      THIndexTensor_(nElement)(LIBRARY_STATE broadcasted.begin()->second.get());
  THLongStorage_set(indexerSize, 0, dimIndexingElements);

  for (auto &broadcast : broadcasted) {
    THIndexTensor *contig =
        THIndexTensor_(newContiguous)(LIBRARY_STATE broadcast.second.get());
    THPPointer<THIndexTensor> flat(
        THIndexTensor_(newView)(LIBRARY_STATE contig, indexerSize));
  ...
  THTensor_(calculateAdvancedIndexingOffsets)(
      LIBRARY_STATE cudaIndices, indexed, baseOffset, indexers.data());

  return cudaIndices;
#else
  THIndexTensor *linearIndices =
      THIndexTensor_(newWithSize1d)(LIBRARY_STATE indexingElements);
  long baseOffset = THTensor_(storageOffset)(LIBRARY_STATE indexed);
  for (ptrdiff_t i = 0; i < indexingElements; ++i) {
    long linearIdx =
        THPTensor_(_indexToOffset)(indexed, flattenedBroadcasters, i);
...
static bool THPTensor_(_advancedIndexCommonInit)(
    PyObject *index, THTensorPtr &indexed,
    std::unordered_map<Py_ssize_t, THPPointer<THIndexTensor>> &broadcasted,
    THIndexTensor **linearIndices, THTensor **flattened) {
  ...
  // Precondition: tresult points to the Tensor we are indexing, and is also
  // where
  ...
  if (!THPTensor_(_convertToTensorIndexers)(index, indexed, sequenceLength,
                                            broadcasted)) {
    return false;
  }
  ...
  *linearIndices =
      THPTensor_(_calculateLinearIndices)(indexed, sequenceLength, broadcasted);

  *flattened = THTensor_(newWithStorage1d)(
      LIBRARY_STATE THTensor_(storage)(LIBRARY_STATE indexed.get()), 0,
      THStorage_(size)(
          LIBRARY_STATE THTensor_(storage)(LIBRARY_STATE indexed.get())),
      1);
...
static void
    THPTensor_(_advancedIndexCommonCleanup)(THIndexTensor *linearIndices,
                                            THTensor *flattened) {
  if (linearIndices)
    THIndexTensor_(free)(LIBRARY_STATE linearIndices);
  if (flattened)
    THTensor_(free)(LIBRARY_STATE flattened);
}

static bool THPTensor_(_advancedIndexGet)(PyObject *index,
                                          THTensorPtr &tresult) {
  std::unordered_map<Py_ssize_t, THPPointer<THIndexTensor>> broadcasted;
  ...
  if (success) {
    ...
    // Index Select makes a copy of the storage, thus it is enforcing NumPy
    // semantics, which
    ...
    // Finally, we need to calculate the appropriate shape of the output
    // Tensor
    // The size at each dimension is unmodified from the input Tensor, except
    // where
    // there are advanced indexers. In this case, the n dimensions containing
    // adjacent
    ...
    // First, calculate the number of dimensions of the output shape. This is
    // the
    // number of non-advanced indexed dimensions + the number of dimensions in
    // the
    // broadcast Tensor
    int baseDims =
        THTensor_(nDimension)(LIBRARY_STATE tresult.get()) - broadcasted.size();

    // Fast path, if we have ndim advanced indexers, the output shape is
    // simply
    // the
    // broadcast shape
    if (baseDims == 0) {
      ...
      THTensor_(resizeNd)(
          ...
          iter->second.get()->size, NULL);
    } else {
      // We have at least one dimension that is not part of advanced indexing.
      // This
      // implementation is pretty much shit, there might be a better way of
      // doing this...
      THIndexTensor *broadcastShape = broadcasted.begin()->second.get();

      int indexedDims =
          THIndexTensor_(nDimension)(LIBRARY_STATE broadcastShape);
      THLongStorage *outputShape =
          THLongStorage_newWithSize(baseDims + indexedDims);
      ...
      while (outputDimPtr != baseDims + indexedDims) {
        ...
        if (iter == broadcasted.end()) {
          outputShape->data[outputDimPtr] =
              THTensor_(size)(LIBRARY_STATE tresult.get(), baseDimPtr);
          ++baseDimPtr;
          ++outputDimPtr;
        } else if (!insertedSubspace) {
          for (int dim = 0; dim < indexedDims; ++dim) {
            outputShape->data[outputDimPtr] =
                THIndexTensor_(size)(LIBRARY_STATE iter->second.get(), dim);
            ++outputDimPtr;
          }
      ...
      THTensor_(resizeNd)(LIBRARY_STATE result, baseDims + indexedDims,
                          outputShape->data, NULL);
...
static bool THPTensor_(_advancedIndexSet)(PyObject *index, THTensorPtr &dest,
                                          PyObject *src) {
  std::unordered_map<Py_ssize_t, THPPointer<THIndexTensor>> broadcasted;
  THIndexTensor *linearIndices = NULL;
  THTensor *flattened = NULL;
  ...
  if (success) {
    if (THPUtils_(checkReal)(src)) {
      ...
      // Because we are doing an index copy, we need to make sure of two
      // things:
      // 1. the src Tensor is 1D and
      // 2. the src is made contiguous before being flattened into a 1D view,
      // if
      // necessary

      THTensor *contiguous =
          THTensor_(newContiguous)(LIBRARY_STATE((THPTensor *)src)->cdata);
      THTensor *cviewed = THTensor_(newWithStorage1d)(
          LIBRARY_STATE THTensor_(storage)(LIBRARY_STATE contiguous),
          THTensor_(storageOffset)(LIBRARY_STATE contiguous),
          THTensor_(nElement)(LIBRARY_STATE contiguous), 1);
      ...
      THPUtils_setError(
          "can't assign %s to a " THPTensorStr " using a LongTensor "
...
static bool THPTensor_(_advancedIndexAdd)(PyObject *index, THTensorPtr &dest,
                                          THTensorPtr &src) {
  std::unordered_map<Py_ssize_t, THPPointer<THIndexTensor>> broadcasted;
  THIndexTensor *linearIndices = NULL;
  THTensor *flattened = NULL;
...
static bool THPTensor_(_advancedIndexSelect)(PyObject *index, THTensorPtr &dest,
                                             THTensorPtr &src) {
  std::unordered_map<Py_ssize_t, THPPointer<THIndexTensor>> broadcasted;
  THIndexTensor *linearIndices = NULL;
  THTensor *flattened = NULL;
...
static PyObject *THPTensor_(advancedIndexAdd)(THPTensor *self, PyObject *args) {
  ...
  THPUtils_assert(PyTuple_GET_SIZE(args) == 2,
                  "advancedIndexAdd takes exactly two "
                  "arguments (%d given)",
                  (int)PyTuple_GET_SIZE(args));

  THPUtils_assert(
      THPTensor_(_checkAdvancedIndexing)(self, PyTuple_GET_ITEM(args, 0)),
      "first argument must be an indexer that triggers advanced indexing");

  THPUtils_assert(THPTensor_(Check)(PyTuple_GET_ITEM(args, 1)),
                  "Second argument "
                  "must be a Tensor");
  ...
  bool success = THPTensor_(_advancedIndexAdd)(PyTuple_GET_ITEM(args, 0), dest,
                                               gradOutput);
  if (!success) {
    return NULL;
  }
...
// indices
static PyObject *THPTensor_(advancedIndexSelect)(THPTensor *self,
                                                 PyObject *args) {
  HANDLE_TH_ERRORS

  THPUtils_assert(PyTuple_GET_SIZE(args) == 1,
                  "advancedIndexSelect takes exactly one "
                  "argument (%d given)",
                  (int)PyTuple_GET_SIZE(args));
  ...
  bool success =
      THPTensor_(_advancedIndexSelect)(PyTuple_GET_ITEM(args, 0), dest, src);
  if (!success) {
    return NULL;
  }
...
// Handles indexing into a Tensor given a tuple, ellipses, sequence, etc.
// index
static bool THPTensor_(_index)(THPTensor *self, PyObject *index,
  ...
  if (PyTuple_Check(index)) {
    // num_indexers is the number of indexing objects in the tuple,
    ...
    for (int i = 0; i < num_indexers; i++) {
      ...
      if (indexer == Py_Ellipsis) {
        if (ellipsis_pos != -1)
          throw std::runtime_error("ellipsis can be used at most once");
        ellipsis_pos = i;
        num_effective_indexers--;
      }
      if (indexer == Py_None) {
    ...
    // Loop through the indices and perform the indiviudal indexing at each
    // dim
    bool valid = true;
    for (int indexTuplePos = 0; indexTuplePos < num_indexers; indexTuplePos++) {
      if (indexTuplePos == ellipsis_pos) {
        ...
        // Note that the presence of the Ellipsis shifts the "indexed" dim by
        // the number
        // of dimensions minus the number of effective indexers
        if (tresult)
          indexed_dim += (num_tensor_dim - num_effective_indexers);
        continue;
      }
      PyObject *indexer = PyTuple_GET_ITEM(index, indexTuplePos);
      valid = THPTensor_(_indexOnce)(indexer, indexed_dim, tresult, sresult,
                                     storage_offset);
      if (!valid) {
        tresult = NULL;
    ...
    // The result of indexing with an ellipsis only is just the entire
    // existing
    // Tensor
    return true;
  } else {
    // index is a scalar, perform the indexing once on the 0th-dimension
    if (THPTensor_(_indexOnce)(index, indexed_dim, tresult, sresult,
                               storage_offset))
      return true;
  }

  PyErr_Format(PyExc_TypeError, "Performing basic indexing on a tensor and "
                                "encountered an error indexing dim %d "
                                "with an object of type %s. The only supported "
                                "types are integers, slices, "
#ifdef WITH_NUMPY
                                "numpy scalars, "
#endif
...
static PyObject *THPTensor_(getValue)(THPTensor *self, PyObject *index) {
...
#if defined(THC_GENERIC_FILE)
  THCPByteTensor *mask =
      THCPByteTensor_Check(index) ? (THCPByteTensor *)index : NULL;
  THCPAutoGPU __gpu_guard(NULL, (PyObject *)self);
#elif defined(THD_GENERIC_FILE)
  THDPByteTensor *mask =
      THDPByteTensor_Check(index) ? (THDPByteTensor *)index : NULL;
#else
  THPByteTensor *mask =
      THPByteTensor_Check(index) ? (THPByteTensor *)index : NULL;
#endif
  if (mask) {
  ...
  if (THPIndexTensor_Check(index)) {
    ...
    THTensor_(indexSelect)(LIBRARY_STATE index_result.get(), self->cdata, 0,
                           index_t);
...
// Check and see if the indexing object triggers advanced indexing semantics
  ...
  if (sresult) {
    if (force_tensor) {
      return THPTensor_(New)(THTensor_(newWithStorage1d)(
          LIBRARY_STATE sresult, storage_offset, 1, -1));
    } else {
      return THPUtils_(newReal)(
          THStorage_(get)(LIBRARY_STATE sresult, storage_offset));
    }
  }
  THPUtils_setError(
      "An unknown error has occurred when indexing a tensor "
...
template <bool force_tensor>
static int THPTensor_(setValue)(THPTensor *self, PyObject *index,
                                PyObject *value) {
  HANDLE_TH_ERRORS
  ...
  if (mask) {
    if (THPUtils_(checkReal)(value)) {
      ...
      THTensor_(maskedCopy)(LIBRARY_STATE self->cdata, mask->cdata,
                            ((THPTensor *)value)->cdata);
    } else {
  ...
  if (THPIndexTensor_Check(index)) {
    ...
    if (THPUtils_(checkReal)(value)) {
      ...
      THTensor_(indexCopy)(LIBRARY_STATE self->cdata, 0, index_t,
                           ((THPTensor *)value)->cdata);
    } else {
      THPUtils_setError(
          "can't assign %s to a " THPTensorStr " using a LongTensor "
...
// Check and see if the indexing object triggers advanced indexing semantics
  ...
  if (sresult) {
    if (!force_tensor) {
      ...
      THStorage_(set)(LIBRARY_STATE sresult, storage_offset,
                      THPUtils_(unpackReal)(value));
      return 0;
    } else {
      tresult = THTensor_(newWithStorage1d)(LIBRARY_STATE sresult,
                                            storage_offset, 1, -1);
  ...
  if (tresult) {
    if (THPUtils_(checkReal)(value)) {
#ifndef TH_REAL_IS_HALF
      THTensor_(fill)(LIBRARY_STATE tresult.get(),
                      THPUtils_(unpackReal)(value));
#else
      throw std::runtime_error(
          "torch.HalfTensors don't support scalar assignments");
#endif
    } else {
      ...
      if (!tmp)
        return -1;
      if (!THPCopy(THTensor_(copy_functions), (PyObject *)tmp.get(), value,
                   false, false)) {
        return -1;
      }
    }
...
static PyMappingMethods THPTensor_(mappingmethods) = {
    (lenfunc)THPTensor_(length), (binaryfunc)THPTensor_(getValue) < false >,
    (objobjargproc)THPTensor_(setValue) < false > };

// TODO: implement equality
PyTypeObject THPTensorType = {
    PyVarObject_HEAD_INIT(NULL, 0) "torch._C." THPTensorBaseStr, /* tp_name */
...
typedef struct { PyObject_HEAD } THPTensorStateless;

PyTypeObject THPTensorStatelessType = {
    PyVarObject_HEAD_INIT(NULL, 0) "torch._C." THPTensorBaseStr
                                   ".stateless", /* tp_name */
...
// copy from CPU types
  ...
  THPInsertTensorCopyFunction(h, &THTensor_(copyZFloat));
#elif defined(TH_REAL_IS_ZDOUBLE) || defined(THC_REAL_IS_ZDOUBLE)
  THPInsertTensorCopyFunction(h, &THTensor_(copyZDouble));
#else
...
// copy from GPU types
#if defined(TH_REAL_IS_ZFLOAT) || defined(THC_REAL_IS_ZFLOAT)
  THPInsertTensorCopyFunction(h, &THTensor_(copyCudaZFloat));
#elif defined(TH_REAL_IS_ZDOUBLE) || defined(THC_REAL_IS_ZDOUBLE)
  THPInsertTensorCopyFunction(h, &THTensor_(copyCudaZDouble));
#else
  ...
  THPInsertTensorCopyFunction(h, &THTensor_(copyCudaDouble));
    ...
    !(defined(TH_REAL_IS_ZDOUBLE) || defined(THC_REAL_IS_ZDOUBLE) ||           \
      defined(TH_REAL_IS_ZFLOAT) || defined(THC_REAL_IS_ZFLOAT))
...
#define THCpuTensor_(name) TH_CONCAT_4(TH, Real, Tensor_, name)
...
#if defined(THC_REAL_IS_ZFLOAT) || defined(TH_REAL_IS_ZFLOAT)
  THPInsertTensorCopyFunction(b, &THCpuTensor_(copyCudaZFloat));
#elif defined(THC_REAL_IS_ZDOUBLE) || defined(TH_REAL_IS_ZDOUBLE)
  THPInsertTensorCopyFunction(b, &THCpuTensor_(copyCudaZDouble));
#else
  ...
  THPInsertTensorCopyFunction(b, &THCpuTensor_(copyCudaDouble));
    ...
    !(defined(TH_REAL_IS_ZDOUBLE) ||                                           \
              defined(THC_REAL_IS_ZDOUBLE) ||                                  \
                      defined(TH_REAL_IS_ZFLOAT) ||                            \
                              defined(THC_REAL_IS_ZFLOAT))
...
#define THPCpuTensorType TH_CONCAT_3(THP, Real, TensorType)
  ...
  THDPInsertCopyFunctionFromMaster(h, &THDTensor_(copyFromMaster),
                                   &THPCpuTensorType);
...
bool THPTensor_(postInit)(PyObject *module) {
  THPTensorClass =
      PyObject_GetAttrString(module, (char *)TH_CONCAT_STRING_2(Real, Tensor));
  if (!THPTensorClass)
    return false;
