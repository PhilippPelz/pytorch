...
void THDTensor_(gather)(THDTensor *self, THDTensor *src, int dim,
                        THDLongTensor *index) {
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorGather, self, src, dim, index),
      THDState::s_current_worker);
}

void THDTensor_(scatter)(THDTensor *self, int dim, THDLongTensor *index,
                         THDTensor *src) {
  THArgCheck(dim < self->nDimension, 2, "Index dimension is out of bounds");
  THArgCheck(THDLongTensor_nDimension(index) == self->nDimension, 3,
             "Index tensor must have same dimensions as output tensor");
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorScatter, self, dim, index, src),
      THDState::s_current_worker);
}

void THDTensor_(scatterFill)(THDTensor *self, int dim, THDLongTensor *index,
                             real val) {
  THArgCheck(dim < self->nDimension, 2, "Index dimension is out of bounds");
  THArgCheck(THDLongTensor_nDimension(index) == self->nDimension, 3,
             "Index tensor must have same dimensions as output tensor");
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorScatterFill, self, dim, index, val),
      THDState::s_current_worker);
}

THD_API void THDTensor_(scatterAdd)(THDTensor *self, int dim,
                                    THDLongTensor *index, THDTensor *src) {
  THError("scatterAdd not implemented");
}

void THDTensor_(max)(THDTensor *self, THDLongTensor *indices_, THDTensor *src,
  ...
  masterCommandChannel->sendMessage(packMessage(Functions::tensorMax, self,
                                                indices_, src, dimension,
                                                keepdim),
                                    THDState::s_current_worker);
...
void THDTensor_(min)(THDTensor *self, THDLongTensor *indices_, THDTensor *src,
                     int dimension, int keepdim) {
  THArgCheck(dimension >= 0 && dimension < src->nDimension, 2,
             "dimension %d out of range", dimension + TH_INDEX_BASE);
  ...
  masterCommandChannel->sendMessage(packMessage(Functions::tensorMin, self,
                                                indices_, src, dimension,
                                                keepdim),
                                    THDState::s_current_worker);
...
void THDTensor_(kthvalue)(THDTensor *self, THDLongTensor *indices_,
                          THDTensor *src, int64_t k, int dimension,
                          int keepdim) {
  THArgCheck(dimension >= 0 && dimension < src->nDimension, 3,
             "dimension out of range");
  THArgCheck(k > 0 && k <= src->size[dimension], 2,
  ...
  masterCommandChannel->sendMessage(packMessage(Functions::tensorKthvalue, self,
                                                indices_, src, k, dimension,
                                                keepdim),
                                    THDState::s_current_worker);
...
void THDTensor_(mode)(THDTensor *self, THDLongTensor *indices_, THDTensor *src,
                      int dimension, int keepdim) {
  THArgCheck(dimension >= 0 && dimension < src->nDimension, 3,
             "dimension out of range");
  ...
  masterCommandChannel->sendMessage(packMessage(Functions::tensorMode, self,
                                                indices_, src, dimension,
                                                keepdim),
                                    THDState::s_current_worker);
...
void THDTensor_(median)(THDTensor *self, THDLongTensor *indices_,
  ...
  int64_t k =
      (t_size_dim - 1) >> 1; /* take middle or one-before-middle element */
...
void THDTensor_(fill)(THDTensor *tensor, real value) {
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorFill, tensor, value),
      THDState::s_current_worker);
}

void THDTensor_(zero)(THDTensor *r) { THDTensor_(fill)(r, 0); }

void THDTensor_(maskedFill)(THDTensor *tensor, THDByteTensor *mask,
                            real value) {
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorMaskedFill, tensor, mask, value),
      THDState::s_current_worker);
}

void THDTensor_(maskedCopy)(THDTensor *tensor, THDByteTensor *mask,
                            THDTensor *src) {
  if (THDTensor_(nElement)(tensor) != THDByteTensor_nElement(mask))
    THError("Number of elements of destination tensor != Number of elements in "
            "mask");
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorMaskedCopy, tensor, mask, src),
      THDState::s_current_worker);
}

void THDTensor_(maskedSelect)(THDTensor *tensor, THDTensor *src,
                              THDByteTensor *mask) {
  ptrdiff_t numel = THDByteTensor_sumall(mask);
  THDTensor_(resize1d)(tensor, numel);
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorMaskedSelect, tensor, src, mask),
      THDState::s_current_worker);
}

void THDTensor_(nonzero)(THDLongTensor *subscript, THDTensor *tensor) {
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorNonzero, subscript, tensor),
      THDState::s_current_worker);
...
void THDTensor_(indexSelect)(THDTensor *tensor, THDTensor *src, int dim,
                             THDLongTensor *index) {
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorIndexSelect, tensor, src, dim, index),
      THDState::s_current_worker);
}

void THDTensor_(indexCopy)(THDTensor *tensor, int dim, THDLongTensor *index,
                           THDTensor *src) {
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorIndexCopy, tensor, dim, index, src),
      THDState::s_current_worker);
}

void THDTensor_(indexAdd)(THDTensor *tensor, int dim, THDLongTensor *index,
                          THDTensor *src) {
  ptrdiff_t numel = THDLongTensor_nElement(index);
  THArgCheck(index->nDimension == 1, 3, "Index is supposed to be a vector");
  THArgCheck(dim < src->nDimension, 4,
  ...
  THArgCheck(numel == src->size[dim], 4,
             "Number of indices should be equal to source:size(dim)");
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorIndexAdd, tensor, dim, index, src),
      THDState::s_current_worker);
}

void THDTensor_(indexFill)(THDTensor *tensor, int dim, THDLongTensor *index,
                           real val) {
  ptrdiff_t numel = THDLongTensor_nElement(index);
  THArgCheck(index->nDimension == 1, 3, "Index is supposed to be a vector");
  THArgCheck(dim < tensor->nDimension, 4,
             "Indexing dim %d is out of bounds of tensor", dim + TH_INDEX_BASE);
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorIndexFill, tensor, dim, index, val),
      THDState::s_current_worker);
}
...
ptrdiff_t THDTensor_(numel)(THDTensor *t) { return THDTensor_(nElement)(t); }
...
void THDTensor_(diag)(THDTensor *r_, THDTensor *t, int k) {
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorDiag, r_, t, k), THDState::s_current_worker);
}
...
void THDTensor_(eye)(THDTensor *r, int64_t n, int64_t m) {
  ...
  masterCommandChannel->sendMessage(packMessage(Functions::tensorEye, r, n, m),
                                    THDState::s_current_worker);
}

void THDTensor_(range)(THDTensor *r_, accreal xmin, accreal xmax,
                       accreal step1) {
#if defined(TH_REAL_IS_ZFLOAT) || defined(TH_REAL_IS_ZDOUBLE)
  int step = CREAL(step1);
  int xmax1 = CREAL(xmax);
  int xmin1 = CREAL(xmin);
  THArgCheck(step > 0 || step < 0, 3, "step must be a non-null number");
  THArgCheck(((step > 0) && (xmax1 >= xmin1)) ||
                 ((step < 0) && (xmax1 <= xmin1)),
             2, "upper bound and larger bound incoherent with step sign");
  ptrdiff_t size = static_cast<ptrdiff_t>((((xmax1 - xmin1) / step) + 1));
#else
  THArgCheck(step1 > 0 || step1 < 0, 3, "step must be a non-null number");
  THArgCheck(((step1 > 0) && (xmax >= xmin)) || ((step1 < 0) && (xmax <= xmin)),
             2, "upper bound and larger bound incoherent with step sign");
  ptrdiff_t size = static_cast<ptrdiff_t>((((xmax - xmin) / step1) + 1));
  int xmax1 = CREAL(xmax);
  int xmin1 = CREAL(xmin);
#endif
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorRange, r_, xmin1, xmax1, step1),
      THDState::s_current_worker);
}
...
void THDTensor_(sort)(THDTensor *rt_, THDLongTensor *ri_, THDTensor *t,
  ...
  masterCommandChannel->sendMessage(packMessage(Functions::tensorSort, rt_, ri_,
                                                t, dimension, descendingOrder),
                                    THDState::s_current_worker);
}

void THDTensor_(topk)(THDTensor *rt_, THDLongTensor *ri_, THDTensor *t,
...
void THDTensor_(tril)(THDTensor *r_, THDTensor *t, int64_t k) {
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorTril, r_, t, k), THDState::s_current_worker);
}
...
void THDTensor_(triu)(THDTensor *r_, THDTensor *t, int64_t k) {
  ...
  masterCommandChannel->sendMessage(
      packMessage(Functions::tensorTriu, r_, t, k), THDState::s_current_worker);
}
void THDTensor_(cat)(THDTensor *r_, THDTensor *ta, THDTensor *tb,
                     int dimension) {
  THDTensor *inputs[2];
  inputs[0] = ta;
  inputs[1] = tb;
...
void THDTensor_(catArray)(THDTensor *result, THDTensor **inputs, int numInputs,
                          int dimension) {
  THLongStorage *size;
  int64_t offset;
  ...
  THArgCheck(ldimension >= 0, 4, "invalid dimension %d",
...
int THDTensor_(equal)(THDTensor *ta, THDTensor *tb) {
  if (!THDTensor_(isSameSizeAs)(ta, tb))
    return 0;
  masterCommandChannel->sendMessage(packMessage(Functions::tensorEqual, ta, tb),
                                    THDState::s_current_worker);
...
#define TENSOR_IMPLEMENT_LOGICAL(NAME, UPPNAME)                                \
  void THDTensor_(NAME##Value)(THDByteTensor * r_, THDTensor * t,              \
                               real value) {                                   \
  ...
  void THDTensor_(NAME##Tensor)(THDByteTensor * r_, THDTensor * ta,            \
                                THDTensor * tb) {                              \
  ...
  void THDTensor_(NAME##TensorT)(THDTensor * r_, THDTensor * ta,               \
                                 THDTensor * tb) {                             \
...
void THDTensor_(std)(THDTensor *r_, THDTensor *t, int dimension, int biased,
                     int keepdim) {
...
void THDTensor_(var)(THDTensor *r_, THDTensor *t, int dimension, int biased,
                     int keepdim) {
  THArgCheck(dimension >= 0 && dimension < THDTensor_(nDimension)(t), 3,
             "invalid dimension %d", dimension + TH_INDEX_BASE);
...
void THDTensor_(histc)(THDTensor *hist, THDTensor *tensor, int64_t nbins,
  ...
  masterCommandChannel->sendMessage(packMessage(Functions::tensorHistc, hist,
                                                tensor, nbins, minvalue,
                                                maxvalue),
                                    THDState::s_current_worker);
}
...
void THDTensor_(bhistc)(THDTensor *hist, THDTensor *tensor, int64_t nbins,
  ...
  masterCommandChannel->sendMessage(packMessage(Functions::tensorBhistc, hist,
                                                tensor, nbins, minvalue,
                                                maxvalue),
                                    THDState::s_current_worker);
}
