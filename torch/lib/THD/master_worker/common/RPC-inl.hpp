...
template <typename real> inline void _appendScalar(ByteArray &str, real data) {
...
template <typename T>
inline void
__appendData(ByteArray &str, const T &arg, std::false_type is_generator,
...
inline void __appendData(ByteArray &str, const T &arg,
                         std::true_type is_generator, std::false_type is_tensor,
                         std::false_type is_storage) {
...
template <typename T>
inline void __appendData(ByteArray &str, const T &arg,
                         std::false_type is_generator, std::true_type is_tensor,
                         std::false_type is_storage) {
...
template <typename T> inline void _appendData(ByteArray &str, const T &arg) {
  __appendData(str, arg, is_any_of<T, THDGeneratorPtrTypes>(),
...
inline void _appendData(ByteArray &str, const std::vector<T> &arg) {
  ...
  for (std::size_t i = 0; i < l; i++)
    __appendData(str, arg[i], is_any_of<T, THDGeneratorPtrTypes>(),
                 is_any_of<T, THDTensorPtrTypes>(),
                 is_any_of<T, THDStoragePtrTypes>());
}
...
inline void _packIntoString(ByteArray &str, const T &arg,
                            const Args &... args) {
  _appendData(str, arg);
...
inline std::unique_ptr<RPCMessage> packMessage(function_id_type fid,
                                               const Args &... args) {
