...
#define ROW_PTR2(t, r)                                                         \
  (THCTensor_(data)(THCState * state, t) + (r) * (t)->stride[0])
#define COL_PTR2(t, c)                                                         \
  (THCTensor_(data)(THCState * state, t) + (c) * (t)->stride[1])
...
THCudaIntTensor *THCSTensor_(toCSR)(THCState *state, THCIndexTensor *rowIndices,
                                    long dim, long nnz) {
  THCudaIntTensor *csr = THCudaIntTensor_newWithSize1d(state, dim + 1);
  THCudaIntTensor *rowIndicesInt =
      THCudaIntTensor_newWithSize1d(state, rowIndices->size[0]);
  THCudaIntTensor_copyCudaLong(state, rowIndicesInt, rowIndices);
  THCudaSparse_Xcoo2csr(state, THCudaIntTensor_data(state, rowIndicesInt), nnz,
                        dim, THCudaIntTensor_data(state, csr));
...
void THCTensor_(spaddcmul)(THCState *state, THCTensor *r_, THCTensor *t,
                           real value, THCSTensor *src1, THCSTensor *src2) {
  THError("WARNING: Sparse Cuda Tensor op spaddcmul is not implemented");
}

void THCTensor_(spaddcdiv)(THCState *state, THCTensor *r_, THCTensor *t,
                           real value, THCSTensor *src1, THCSTensor *src2) {
  THError("WARNING: Sparse Cuda Tensor op spaddcdiv is not implemented");
}

void THCSTensor_(spaddmm)(THCState *state, THCTensor *r_, real beta,
                          THCTensor *t, real alpha, THCSTensor *sparse_,
                          THCTensor *dense) {
  ...
  THArgCheck(sparse_->nDimensionI == 2, 2, "matrices expected, got %dD tensor",
             sparse_->nDimensionI);
  THArgCheck(sparse_->nDimensionV == 0, 2,
             "scalar values expected, got %dD values", sparse_->nDimensionV);
  THArgCheck(dense->nDimension == 2, 2, "matrices expected, got %dD tensor",
             dense->nDimension);
  ...
  THArgCheck(THCTensor_(size)(state, t, 0) == m, 1,
             "Expected dim 0 size %d, got %d", m,
             THCTensor_(size)(state, t, 0));
  THArgCheck(THCTensor_(size)(state, t, 1) == n, 1,
             "Expected dim 1 size %d, got %d", n,
             THCTensor_(size)(state, t, 1));
  THArgCheck(THCTensor_(size)(state, dense, 0) == k, 3,
             "Expected dim 0 size %d, got %d", k,
             THCTensor_(size)(state, dense, 0));
  ...
  THCudaIntTensor *colIndicesInt =
      THCudaIntTensor_newWithSize1d(state, colIndices->size[0]);
...
#endif
      state, 'n', transpose_dense, m, n, k, nnz, alpha,
      THCTensor_(data)(state, values), THCudaIntTensor_data(state, csr),
      THCudaIntTensor_data(state, colIndicesInt),
      THCTensor_(data)(state, dense_),
      (transpose_dense == 'n' ? dense_->stride[1] : dense_->stride[0]), beta,
      THCTensor_(data)(state, r__), r__->stride[1]);
...
void THCSTensor_(sspaddmm)(THCState *state, THCSTensor *r_, real beta,
                           THCSTensor *t, real alpha, THCSTensor *sparse,
                           THCTensor *dense) {
  THError("WARNING: Sparse Cuda Tensor op sspaddmm is not implemented");
  // TODO Write some kernels
}

void THCSTensor_(hspmm)(THCState *state, THCSTensor *r_, real alpha,
                        THCSTensor *sparse_, THCTensor *dense) {
...
#define THRUST_EXEC(fn, ...) fn(##__VA_ARGS__)
  ...
  THArgCheck(sparse_->nDimensionI == 2, 3, "matrices expected, got %dD tensor",
             sparse_->nDimensionI);
  THArgCheck(sparse_->nDimensionV == 0, 3,
             "scalar values expected, got %dD values", sparse_->nDimensionV);
  THArgCheck(dense->nDimension == 2, 4, "matrices expected, got %dD tensor",
             dense->nDimension);
  ...
  THCIndexTensor *dstIndices =
      THCIndexTensor_(newSelect)(state, spIndices, 0, 0);
  ...
  thrust::device_ptr<indexT> indicesIter(
      THCIndexTensor_(data)(state, dstIndices));
  THRUST_EXEC(thrust::sequence, indicesIter, indicesIter + nnz);
  newSparse->size[0] = nnz;
  THCSTensor_(spaddmm)(state, values, ScalarConvert<int, real>::to(0), values,
                       alpha, newSparse, dense);
...
void THCSTensor_(spcadd)(THCState *state, THCTensor *r_, THCTensor *dense,
                         real value, THCSTensor *sparse) {
  ...
  if (THCSTensor_(isCoalesced)(state, sparse)) {
    ...
    if (sparse->nDimensionV == 0) {
      ...
      THCSTensor_sparseElementwiseKernelScalar<
          TensorCAddOp<real>, unsigned long,
          real><<<grid, block, 0, THCState_getCurrentStream(state)>>>(
          TensorCAddOp<real>(value), V_INFO(r_), I_INFO(indices),
          V_INFO(values), (unsigned long)nnz);
    } else {
      THArgCheck(getApplyGrid(state, nnz * block.x, grid), 1,
                 CUTORCH_DIM_WARNING);

      THCSTensor_sparseElementwiseKernel<
          TensorCAddOp<real>, unsigned long,
          real><<<grid, block, 0, THCState_getCurrentStream(state)>>>(
          TensorCAddOp<real>(value), V_INFO(r_), I_INFO(indices),
          V_INFO(values), (unsigned long)nnz);
    ...
    THLongStorage *r_view_size =
        THLongStorage_newWithSize2(view_rows, view_columns);
...
void THCSTensor_(mul)(THCState *state, THCSTensor *r_, THCSTensor *t,
                      real value) {
  if (r_ == t) {
...
void THCSTensor_(div)(THCState *state, THCSTensor *r_, THCSTensor *t,
                      real value) {
  if (r_ == t) {
    THCTensor *r_values_ = THCSTensor_(newValues)(state, r_);
...
void THCSTensor_(cadd)(THCState *state, THCSTensor *r_, THCSTensor *t,
                       real value, THCSTensor *src) {
...
void THCSTensor_(csub)(THCState *state, THCSTensor *r_, THCSTensor *t,
                       real value, THCSTensor *src) {
  THCSTensor_(cadd)(state, r_, t, ScalarNegate<real>::to(value), src);
}

void THCSTensor_(cmul)(THCState *state, THCSTensor *r_, THCSTensor *t_,
                       THCSTensor *src_) {
  ...
  THCIndexTensor *s_indices_ = THCSTensor_(newIndices)(state, src);
  THCTensor *s_values_ = THCSTensor_(newValues)(state, src);
  THCIndexTensor *r_indices_ =
      THCIndexTensor_(newWithSize2d)(state, nDimI, max_nnz);
  THCTensor *r_values_ =
      THCSTensor_(newValuesWithSizeOf)(state, s_values_, max_nnz);
  ...
  THCSTensor_valueSparseIntersectionKernel<
      TensorMulOp<real>, unsigned long,
      real><<<grid, block, 0, THCState_getCurrentStream(state)>>>(
      TensorMulOp<real>(), I_INFO(r_indices_), I_INFO(t_indices_),
      I_INFO(s_indices_), V_INFO(r_values_), V_INFO(t_values_),
      V_INFO(s_values_), (unsigned long)t_nnz, (unsigned long)s_nnz);
  THCudaCheck(cudaGetLastError());

  THCudaLongStorage *resultNnz = THCudaLongStorage_newWithSize(state, 1);
  THCSTensor_indexSparseIntersectionKernel<
      unsigned long, real><<<1, 1, 0, THCState_getCurrentStream(state)>>>(
      I_INFO(r_indices_), I_INFO(t_indices_), I_INFO(s_indices_),
      (unsigned long)t_nnz, (unsigned long)s_nnz,
      (unsigned long *)resultNnz->data);
  THCudaCheck(cudaGetLastError());
...
#if defined(THCS_REAL_IS_FLOAT) || defined(THCS_REAL_IS_DOUBLE)
void THCSTensor_(pow)(THCState *state, THCSTensor *r_, THCSTensor *t_,
                      real value) {
  if (value == 0) {
