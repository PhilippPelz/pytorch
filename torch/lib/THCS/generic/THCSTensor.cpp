...
int64_t THCSTensor_(size)(THCState *state, const THCSTensor *self, int dim) {
  THArgCheck((dim >= 0) && (dim < self->nDimensionI + self->nDimensionV), 1,
             "dimension %d out of range of %dD tensor", dim + 1,
             THCSTensor_(nDimension)(state, self));
...
THLongStorage *THCSTensor_(newSizeOf)(THCState *state, THCSTensor *self) {
  THLongStorage *size =
      THLongStorage_newWithSize(self->nDimensionI + self->nDimensionV);
  THLongStorage_rawCopy(size, self->size);
  return size;
}

/*** TODO: watch out for memory leaks ***/
THCIndexTensor *THCSTensor_(newIndices)(THCState *state,
                                        const THCSTensor *self) {
  if (self->nnz == 0) {
...
void THCSTensor_(rawResize)(THCState *state, THCSTensor *self, int nDimI,
                            int nDimV, int64_t *size) {
  // Only resize valid sizes into tensor.
  self->size =
      (int64_t *)THRealloc(self->size, sizeof(int64_t) * (nDimI + nDimV));
...
THCSTensor *THCSTensor_(_move)(THCState *state, THCSTensor *self,
                               THCIndexTensor *indices, THCTensor *values) {
  ...
  if (!empty) {
    ...
    THArgCheck(THCIndexTensor_(size)(state, indices, 1) ==
                   THCTensor_(size)(state, values, 0),
               2, "indices and values must have same nnz");
    THArgCheck(THCIndexTensor_(size)(state, indices, 0) == self->nDimensionI, 2,
               "indices has incorrect first dimension, expected %d, got %d",
               self->nDimensionI, THCIndexTensor_(size)(state, indices, 0));
    THArgCheck(THCTensor_(nDimension)(state, values) == self->nDimensionV + 1,
               3,
               "values has incorrect number of dimensions, expected %d, got %d",
               self->nDimensionV + 1, THCTensor_(nDimension)(state, values));
  } else {
...
THCSTensor *THCSTensor_(_set)(THCState *state, THCSTensor *self,
                              THCIndexTensor *indices, THCTensor *values) {
  // Note: Not like torch.set, this is an internal method
  return THCSTensor_(_move)(state, self,
                            THCIndexTensor_(newClone)(state, indices),
                            THCTensor_(newClone)(state, values));
}
...
/* Pointer-copy init */
THCSTensor *THCSTensor_(newWithTensor)(THCState *state, THCIndexTensor *indices,
                                       THCTensor *values) {
  return THCSTensor_(newWithTensorAndSize)(state, indices, values, NULL);
}

THCSTensor *THCSTensor_(newWithTensorAndSize)(
    THCState *state, THCIndexTensor *indices, THCTensor *values,
    THLongStorage *sizes) { // If sizes are not given, it is inferred as max
                            // index of each dim.
  int64_t nDimI, nDimV;
  ...
  if (!sizes) {
    ...
    THCSTensor_(rawResize)(state, self, nDimI, nDimV,
                           THLongTensor_data(computed_sizes));
    ...
    THCSTensor_(rawResize)(state, self, nDimI, nDimV,
                           THLongStorage_data(sizes));
  }
  // NB: by default, we do NOT clone indices/values into the sparse tensor.
  // Efficient API by default!
  THCSTensor_(_move)(state, self,
                     THCIndexTensor_(newWithTensor)(state, indices),
                     THCTensor_(newWithTensor)(state, values));
...
THCSTensor *THCSTensor_(newWithSize2d)(THCState *state, int64_t size0,
                                       int64_t size1) {
...
THCSTensor *THCSTensor_(newWithSize3d)(THCState *state, int64_t size0,
...
THCSTensor *THCSTensor_(newWithSize4d)(THCState *state, int64_t size0,
                                       int64_t size1, int64_t size2,
                                       int64_t size3) {
...
THCSTensor *THCSTensor_(newClone)(THCState *state, THCSTensor *self) {
  THCSTensor *other = (THCSTensor *)THCSTensor_(new)(state);
  THCSTensor_(rawResize)(state, other, self->nDimensionI, self->nDimensionV,
                         self->size);
...
THCSTensor *THCSTensor_(newTranspose)(THCState *state, THCSTensor *self, int d1,
                                      int d2) {
  THCSTensor *other = (THCSTensor *)THCSTensor_(newClone)(state, self);
  THCSTensor_(transpose)(state, other, d1, d2);
  return other;
}

THCTensor *THCSTensor_(newValuesWithSizeOf)(THCState *state, THCTensor *values,
                                            int64_t nnz) {
  THCTensor *new_values;
  if (THCTensor_(nDimension)(state, values) ==
      0) { // values tensor uninitialized
...
int THCSTensor_(isSameSizeAs)(THCState *state, const THCSTensor *self,
                              const THCSTensor *src) {
  if (self->nDimensionI != src->nDimensionI ||
...
int THCSTensor_(isSameSizeAsDense)(THCState *state, const THCSTensor *self,
                                   const THCTensor *src) {
...
THCSTensor *THCSTensor_(resize)(THCState *state, THCSTensor *self,
                                THLongStorage *size) {
  THCSTensor_(rawResize)(state, self, size->size, 0, size->data);
  return self;
}

THCSTensor *THCSTensor_(resizeAs)(THCState *state, THCSTensor *self,
                                  THCSTensor *src) {
  if (!THCSTensor_(isSameSizeAs)(state, self, src)) {
    THCSTensor_(rawResize)(state, self, src->nDimensionI, src->nDimensionV,
                           src->size);
  }
  return self;
}

THCSTensor *THCSTensor_(resize1d)(THCState *state, THCSTensor *self,
                                  int64_t size0) {
  int64_t size[1] = {size0};
  THCSTensor_(rawResize)(state, self, 1, 0, size);
  return self;
...
THCSTensor *THCSTensor_(resize2d)(THCState *state, THCSTensor *self,
...
THCSTensor *THCSTensor_(resize3d)(THCState *state, THCSTensor *self,
...
THCSTensor *THCSTensor_(resize4d)(THCState *state, THCSTensor *self,
                                  int64_t size0, int64_t size1, int64_t size2,
                                  int64_t size3) {
  int64_t size[4] = {size0, size1, size2, size3};
  THCSTensor_(rawResize)(state, self, 4, 0, size);
...
int THCSTensor_(checkGPU)(THCState *state, unsigned int nSparseTensors,
...
void THCTensor_(sparseMask)(THCState *state, THCSTensor *r_, THCTensor *t,
                            THCSTensor *mask) {
  ...
  THCSTensor_(_move)(state, r_, THCIndexTensor_(newClone)(state, maskIndices),
                     rValues);
