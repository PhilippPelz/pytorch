...
template <typename T>
__global__ void multinomialAliasDrawKernel(int size, int64_t *output,
                                           int64_t *j, T *q, int64_t K,
                                           T *uniform, T *bernoulli) {
...
template <typename T>
__global__ void
aliasMultinomialFilter(T *q, T *probs, int64_t *smaller, int64_t *larger,
                       int64_t *J_data, int64_t *larger_short_data,
                       int64_t *smaller_short_data, T one, int64_t inputsize) {
  int64_t idx = blockIdx.x * BLOCK_SIZE + threadIdx.x;
  if (idx < inputsize) {
    larger_short_data[idx] = 0;
    smaller_short_data[idx] = 0;
    J_data[idx] = 0;
    T val = THCNumerics<T>::mul(probs[idx],
                                ScalarConvert<int64_t, T>::to(inputsize));
...
template <typename T>
__global__ void condDiv(T *q, int64_t *j, int64_t inputsize, T q_max) {
  int64_t idx = blockIdx.x * BLOCK_SIZE + threadIdx.x;
...
__global__ void sampleMultinomialOnce(int64_t *dest, int64_t distributions,
...
sampleMultinomialWithReplacement(curandStateMtgp32 *state, int totalSamples,
                                 int64_t *dest, int64_t distributions,
                                 int categories, T *normDistPrefixSum) {
...
__global__ void sampleMultinomialWithoutReplacement(
    curandStateMtgp32 *state, int totalSamples, int sample, int64_t *dest,
    int64_t distributions, int categories, T *origDist, T *normDistPrefixSum) {
  // At the moment, each warp computes one sample value in the binary
  // search due to divergence. It seems possible to compute multiple
  // values and limit divergence though later on. However, no matter
  // what, all block threads must participate in the curand_uniform
  ...
  for (int64_t curDistBase = blockIdx.x * blockDim.y;
       curDistBase < distributions; curDistBase += gridDim.x * blockDim.y) {
...
template <typename T>
__global__ void aliasMultinomialSetup(int64_t *j, T *q, int64_t inputsize,
                                      int64_t *smaller, int64_t *larger,
                                      int small_c, int large_c) {
