  ...
  struct Tensor_##NAME##_##REAL##_Op {                                         \
    ...
    __device__ __forceinline__ void operator()(real *v) const {                \
      *v = CFUNC(*v);                                                          \
                                                                               ...
                                                                               \
  void THCTensor_(NAME)(THCState * state, THCTensor * self_,                   \
                        THCTensor * src) {                                     \
    THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src));              \
    if (self_ == src) {                                                        \
  ...
  struct ZTensor_##NAME##_##REAL##_Op {                                        \
    __device__ __forceinline__ void operator()(part *out, real *in) const {    \
      *out = CFUNC(*in);                                                       \
                                                                               ...
                                                                               \
  void THCTensor_(z##NAME)(THCState * state, THCPartTensor * self_,            \
                           THCTensor * src) {                                  \
      ...
      for (d = 0; d < self_->nDimension; d++) {                                \
        if (self_->size[d] != src->size[d]) {                                  \
          ...
          break;                                                               \
                                                                               ...
                                                                               \
    if (!isSame)                                                               \
      THCPartTensor_(resizeNd)(state, self_, src->nDimension, src->size,       \
                               NULL);                                          \
    if (!THC_pointwiseApply2(state, self_, src,                                \
                             ZTensor_##NAME##_##REAL##_Op())) {                \
      THArgCheck(false, 2, CUTORCH_DIM_WARNING);                               \
    }                                                                          \
    THCudaCheck(cudaGetLastError());                                           \
  }
...
THC_API void THCTensor_(lerp)(THCState *state, THCTensor *result, THCTensor *a,
  ...
  THArgCheck(THCTensor_(nElement)(state, a) == THCTensor_(nElement)(state, b),
...
void THCTensor_(clamp)(THCState *state, THCTensor *self_, THCTensor *src,
                       real min_value, real max_value) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src));
  if (self_ == src) {
    if (!THC_pointwiseApply1(state, self_,
                             TensorClampOp<real>(min_value, max_value))) {
      THArgCheck(false, 2, CUTORCH_DIM_WARNING);
    }
  } else {
    THCTensor_(resizeAs)(state, self_, src);
...
THC_API void THCTensor_(cross)(THCState *state, THCTensor *self, THCTensor *x,
                               THCTensor *y, int dimension) {
  ...
  for (i = 0; i < nd; i++) {
    THArgCheck(THCTensor_(size)(state, x, i) == THCTensor_(size)(state, y, i),
               1, "dimension %i of x and y does not match", i);
    if (dimension < 0 && THCTensor_(size)(state, x, i) == 3) {
      dimension = i;
    }
  }

  THArgCheck(dimension >= 0 && dimension < nd, 3, "dimension %d out of range",
             dimension + 1);
  ...
  if (!THC_pointwiseApply3(state, nself, nx, ny,
                           TensorCrossOp<real>(sx, sy, so))) {
    THArgCheck(false, 2, CUTORCH_DIM_WARNING);
  }
...
void THCTensor_(pow)(THCState *state, THCTensor *self_, THCTensor *src,
                     real value) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src));
  if (self_ == src) {
...
void THCTensor_(tpow)(THCState *state, THCTensor *self_, real value,
                      THCTensor *src) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src));
  if (self_ == src) {
...
THC_API void THCTensor_(cadd)(THCState *state, THCTensor *self_,
                              THCTensor *src1, real value, THCTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 3, self_, src1, src2));
  THArgCheck(THCTensor_(nElement)(state, src1) ==
                 THCTensor_(nElement)(state, src2),
             3, "sizes do not match");
...
THC_API void THCTensor_(csub)(THCState *state, THCTensor *self_,
                              THCTensor *src1, real value, THCTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 3, self_, src1, src2));
  THArgCheck(THCTensor_(nElement)(state, src1) ==
                 THCTensor_(nElement)(state, src2),
  ...
  if (self_ == src1) {
    if (value == ScalarConvert<int, real>::to(1)) {
      ...
      if (!THC_pointwiseApply2(
              state, self_, src2,
              TensorCAddOp<real>(ScalarNegate<real>::to(value)))) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    }
  } else {
    ...
    if (value == ScalarConvert<int, real>::to(1)) {
      ...
      // self = src1 - value * src2
      if (!THC_pointwiseApply3(
              state, self_, src1, src2,
              TensorCAddOp<real>(ScalarNegate<real>::to(value)))) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    }
...
THC_API void THCTensor_(cmul)(THCState *state, THCTensor *self_,
...
THC_API void THCTensor_(cpow)(THCState *state, THCTensor *self_,
                              THCTensor *src1, THCTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 3, self_, src1, src2));
  THArgCheck(THCTensor_(nElement)(state, src1) ==
                 THCTensor_(nElement)(state, src2),
...
THC_API void THCTensor_(cdiv)(THCState *state, THCTensor *self_,
                              THCTensor *src1, THCTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 3, self_, src1, src2));
  THArgCheck(THCTensor_(nElement)(state, src1) ==
                 THCTensor_(nElement)(state, src2),
...
THC_API void THCTensor_(zcadd)(THCState *state, THCTensor *self_,
                              THCTensor *src1, real value, THCPartTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src1));
  THArgCheck(THCTensor_(nElement)(state, src1) ==
                 THCPartTensor_(nElement)(state, src2),
             3, "sizes do not match");

  if (self_ == src1) {
    if (value == ScalarConvert<int, real>::to(1)) {
      // self += src2
      if (!THC_pointwiseApply2(state, self_, src2, TensorZAddOp<real,part>())) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    } else {
      // self += value * src2
      if (!THC_pointwiseApply2(state, self_, src2, TensorZCAddOp<real,part>(value))) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    }
  } else {
    ...
    if (value == ScalarConvert<int, real>::to(1)) {
      // self = src1 + src2
      if (!THC_pointwiseApply3(state, self_, src1, src2, TensorZAddOp<real,part>())) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    } else {
      // self = src1 + value * src2
      if (!THC_pointwiseApply3(state, self_, src1, src2,
                               TensorZCAddOp<real,part>(value))) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    }
  }
...
THC_API void THCTensor_(zcsub)(THCState *state, THCTensor *self_,
                              THCTensor *src1, real value, THCPartTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src1));
  THArgCheck(THCTensor_(nElement)(state, src1) ==
                 THCPartTensor_(nElement)(state, src2),
  ...
  if (self_ == src1) {
    if (value == ScalarConvert<int, real>::to(1)) {
      // self -= src2
      if (!THC_pointwiseApply2(state, self_, src2, TensorZSubOp<real,part>())) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    } else {
      // self += -value * src2
      if (!THC_pointwiseApply2(
              state, self_, src2,
              TensorZCAddOp<real,part>(ScalarNegate<real>::to(value)))) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    }
  } else {
    ...
    if (value == ScalarConvert<int, real>::to(1)) {
      // self = src1 - src2
      if (!THC_pointwiseApply3(state, self_, src1, src2, TensorZSubOp<real,part>())) {
        THArgCheck(false, 2, CUTORCH_DIM_WARNING);
      }
    } else {
      // self = src1 - value * src2
...
THC_API void THCTensor_(zcmul)(THCState *state, THCTensor *self_,
                              THCTensor *src1, THCPartTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src1));
  THArgCheck(THCTensor_(nElement)(state, src1) ==
                 THCPartTensor_(nElement)(state, src2),
             3, "sizes do not match");
  ...
  if (self_ == src1) {
    // self *= src2
    if (!THC_pointwiseApply2(state, self_, src2, TensorZMulOp<real,part>())) {
      THArgCheck(false, 2, CUTORCH_DIM_WARNING);
    }
  } else {
    THCTensor_(resizeAs)(state, self_, src1);
    ...
    // self = src1 * src2
    if (!THC_pointwiseApply3(state, self_, src1, src2, TensorZMulOp<real,part>())) {
      THArgCheck(false, 2, CUTORCH_DIM_WARNING);
    }
  }
...
THC_API void THCTensor_(zcdiv)(THCState *state, THCTensor *self_,
                              THCTensor *src1, THCPartTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src1));
  THArgCheck(THCTensor_(nElement)(state, src1) ==
                 THCPartTensor_(nElement)(state, src2),
  ...
  if (self_ == src1) {
    // self /= src2
    if (!THC_pointwiseApply2(state, self_, src2, TensorZDivOp<real,part>())) {
      THArgCheck(false, 2, CUTORCH_DIM_WARNING);
    }
  } else {
    THCTensor_(resizeAs)(state, self_, src1);
    ...
    // self = src1 / src2
    if (!THC_pointwiseApply3(state, self_, src1, src2, TensorZDivOp<real,part>())) {
      THArgCheck(false, 2, CUTORCH_DIM_WARNING);
    }
  }
...
THC_API void THCTensor_(addcmul)(THCState *state, THCTensor *self_,
                                 THCTensor *t, real value, THCTensor *src1,
                                 THCTensor *src2) {
  ...
  if (self_ != t) {
    ...
    THArgCheck(THCTensor_(nElement)(state, self_) ==
                   THCTensor_(nElement)(state, src1),
...
THC_API void THCTensor_(addcdiv)(THCState *state, THCTensor *self_,
                                 THCTensor *t, real value, THCTensor *src1,
                                 THCTensor *src2) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 4, self_, t, src1, src2));
  if (self_ != t) {
...
THC_API void THCTensor_(clshift)(THCState *state, THCTensor *self_,
                                 THCTensor *src1, THCTensor *src2) {
#if defined(THC_REAL_IS_HALF)
...
THC_API void THCTensor_(crshift)(THCState *state, THCTensor *self_,
                                 THCTensor *src1, THCTensor *src2) {
#if defined(THC_REAL_IS_HALF)
...
THC_API void THCTensor_(cremainder)(THCState *state, THCTensor *self,
  ...
  if (self == src1) {
    ...
    THCTensor_(resizeAs)(state, self, src1);
    if (!THC_pointwiseApply3(state, self, src1, src2,
                             TensorCRemainderOp<real>())) {
      THArgCheck(false, 2, CUTORCH_DIM_WARNING);
    }
  }
#endif
...
THC_API void THCTensor_(cbitand)(THCState *state, THCTensor *self_,
                                 THCTensor *src1, THCTensor *src2) {
#if defined(THC_REAL_IS_HALF) || defined(THC_REAL_IS_FLOAT) ||                 \
    defined(THC_REAL_IS_DOUBLE)
...
THC_API void THCTensor_(cbitor)(THCState *state, THCTensor *self_,
                                THCTensor *src1, THCTensor *src2) {
#if defined(THC_REAL_IS_HALF) || defined(THC_REAL_IS_FLOAT) ||                 \
    defined(THC_REAL_IS_DOUBLE)
...
THC_API void THCTensor_(cbitxor)(THCState *state, THCTensor *self_,
                                 THCTensor *src1, THCTensor *src2) {
#if defined(THC_REAL_IS_HALF) || defined(THC_REAL_IS_FLOAT) ||                 \
    defined(THC_REAL_IS_DOUBLE)
