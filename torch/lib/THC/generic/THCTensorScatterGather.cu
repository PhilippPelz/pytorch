...
#define RUN(TYPE, DIMS, REAL)                                                  \
  THCudaTensor_gatherKernel<                                                   \
      TYPE, REAL, DIMS><<<grid, block, 0, THCState_getCurrentStream(state)>>>( \
      tensorInfo, srcInfo, indexInfo, dim, (TYPE)totalElements);

void THCTensor_(gather)(THCState *state, THCTensor *tensor, THCTensor *src,
                        int dim, THCudaLongTensor *index) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, tensor, src));
  THCAssertSameGPU(THCudaLongTensor_checkGPU(state, 1, index));

  THArgCheck(THCTensor_(nDimension)(state, src) ==
                 THCTensor_(nDimension)(state, tensor),
             2, "Input tensor must have same dimensions as output tensor");
  THArgCheck(dim >= 0 && dim < THCTensor_(nDimension)(state, tensor), 3,
             "Index dimension is out of bounds");
  THArgCheck(THCudaLongTensor_nDimension(state, index) ==
                 THCTensor_(nDimension)(state, src),
             4, "Index tensor must have same dimensions as input tensor");
  ...
  for (int d = 0; d < THCTensor_(nDimension)(state, tensor); d++) {
    if (d != dim) {
      THArgCheck(THCTensor_(size)(state, tensor, d) ==
                     THCTensor_(size)(state, src, d),
                 2, "Input tensor must have same size as output tensor apart "
                    "from the specified dimension");
    }
  }

  THArgCheck(THCTensor_(nDimension)(state, tensor) <= MAX_CUTORCH_DIMS, 1,
             CUTORCH_DIM_WARNING);
...
#define RUN(TYPE, DIMS, REAL)                                                  \
  THCudaTensor_scatterKernel<                                                  \
      TYPE, REAL, DIMS><<<grid, block, 0, THCState_getCurrentStream(state)>>>( \
      tensorInfo, srcInfo, indexInfo, dim, (TYPE)totalElements);

void THCTensor_(scatter)(THCState *state, THCTensor *tensor, int dim,
                         THCudaLongTensor *index, THCTensor *src) {
  ...
  THArgCheck(THCudaLongTensor_nDimension(state, index) ==
                 THCTensor_(nDimension)(state, src),
             3, "Index tensor must have same dimensions as input tensor");
  THArgCheck(THCTensor_(nDimension)(state, src) ==
                 THCTensor_(nDimension)(state, tensor),
             4, "Input tensor must have same dimensions as output tensor");
  ...
  for (int d = 0; d < THCTensor_(nDimension)(state, tensor); d++) {
    if (d != dim) {
      THArgCheck(THCTensor_(size)(state, tensor, d) ==
                     THCTensor_(size)(state, src, d),
                 4, "Input tensor must have same size as output tensor apart "
                    "from the specified dimension");
    }
  }
...
void THCTensor_(scatterAdd)(THCState *state, THCTensor *tensor, int dim,
                            THCudaLongTensor *index, THCTensor *src) {
#if !(defined(THC_REAL_IS_ZFLOAT) || defined(THC_REAL_IS_ZDOUBLE))
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, tensor, src));
  THCAssertSameGPU(THCudaLongTensor_checkGPU(state, 1, index));
...
#define RUN(TYPE, DIMS, REAL)                                                  \
  THCudaTensor_scatterFillKernel<                                              \
      TYPE, REAL, DIMS><<<grid, block, 0, THCState_getCurrentStream(state)>>>( \
      tensorInfo, indexInfo, value, dim, (TYPE)totalElements);

void THCTensor_(scatterFill)(THCState *state, THCTensor *tensor, int dim,
                             THCudaLongTensor *index, real value) {
  ...
  THArgCheck(THCudaLongTensor_nDimension(state, index) ==
                 THCTensor_(nDimension)(state, tensor),
             3, "Index tensor must have same dimensions as output tensor");

  for (int d = 0; d < THCTensor_(nDimension)(state, tensor); d++) {
    if (d != dim) {
      THArgCheck(THCTensor_(size)(state, tensor, d) ==
                     THCudaLongTensor_size(state, index, d),
                 4, "Index tensor must have same size as output tensor apart "
                    "from the specified dimension");
    }
  }
