...
THC_API accreal THCTensor_(dot)(THCState *state, THCTensor *self,
                                THCTensor *src) {
  ...
  accreal result = THCudaBlas_Sdot(state, THCTensor_(nElement)(state, self),
                                   THCTensor_(data)(state, self), 1,
                                   THCTensor_(data)(state, src), 1);
#elif defined(THC_REAL_IS_DOUBLE)
  accreal result = THCudaBlas_Ddot(state, THCTensor_(nElement)(state, self),
                                   THCTensor_(data)(state, self), 1,
                                   THCTensor_(data)(state, src), 1);
#elif defined(THC_REAL_IS_ZDOUBLE)
  accreal result = THCudaBlas_Zdot(state, THCTensor_(nElement)(state, self),
                                   THCTensor_(data)(state, self), 1,
                                   THCTensor_(data)(state, src), 1);
#elif defined(THC_REAL_IS_ZFLOAT)
  accreal result = THCudaBlas_Cdot(state, THCTensor_(nElement)(state, self),
                                   THCTensor_(data)(state, self), 1,
                                   THCTensor_(data)(state, src), 1);
#elif defined(THC_REAL_IS_HALF)
  accreal result = THCudaBlas_Hdot(state, THCTensor_(nElement)(state, self),
                                   THCTensor_(data)(state, self), 1,
                                   THCTensor_(data)(state, src), 1);
#endif
...
THC_API void THCTensor_(addmv)(THCState *state, THCTensor *r_, real beta,
                               THCTensor *t, real alpha, THCTensor *mat,
                               THCTensor *vec) {
#if defined(THC_REAL_IS_FLOAT) || defined(THC_REAL_IS_DOUBLE) ||               \
    defined(THC_REAL_IS_ZFLOAT) || defined(THC_REAL_IS_ZDOUBLE) ||             \
    defined(THC_REAL_IS_HALF)
  ...
  if (mat->stride[0] == 1) {
#ifdef THC_REAL_IS_FLOAT
    THCudaBlas_Sgemv(state, 'n', mat->size[0], mat->size[1], alpha,
                     THCTensor_(data)(state, mat), mat->stride[1],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_DOUBLE)
    THCudaBlas_Dgemv(state, 'n', mat->size[0], mat->size[1], alpha,
                     THCTensor_(data)(state, mat), mat->stride[1],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_ZFLOAT)
    THCudaBlas_Cgemv(state, 'n', mat->size[0], mat->size[1], alpha,
                     THCTensor_(data)(state, mat), mat->stride[1],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_ZDOUBLE)
    THCudaBlas_Zgemv(state, 'n', mat->size[0], mat->size[1], alpha,
                     THCTensor_(data)(state, mat), mat->stride[1],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#endif
...
#ifdef THC_REAL_IS_FLOAT
    THCudaBlas_Sgemv(state, 't', mat->size[1], mat->size[0], alpha,
                     THCTensor_(data)(state, mat), mat->stride[0],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_DOUBLE)
    THCudaBlas_Dgemv(state, 't', mat->size[1], mat->size[0], alpha,
                     THCTensor_(data)(state, mat), mat->stride[0],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_ZFLOAT)
    THCudaBlas_Cgemv(state, 't', mat->size[1], mat->size[0], alpha,
                     THCTensor_(data)(state, mat), mat->stride[0],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_ZDOUBLE)
    THCudaBlas_Zgemv(state, 't', mat->size[1], mat->size[0], alpha,
                     THCTensor_(data)(state, mat), mat->stride[0],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#endif
...
#ifdef THC_REAL_IS_FLOAT
    THCudaBlas_Sgemv(state, 't', mat->size[1], mat->size[0], alpha,
                     THCTensor_(data)(state, cmat), cmat->stride[0],
                     THCTensor_(data)(state, vec), vec->stride[0], beta,
                     THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_DOUBLE)
    THCudaBlas_Dgemv(state, 't', mat->size[1], mat->size[0], alpha,
...
THC_API void THCTensor_(addr)(THCState *state, THCTensor *r_, real beta,
                              THCTensor *t, real alpha, THCTensor *vec1,
                              THCTensor *vec2) {
#if defined(THC_REAL_IS_FLOAT) || defined(THC_REAL_IS_DOUBLE) ||               \
    defined(THC_REAL_IS_HALF) || defined(THC_REAL_IS_ZFLOAT) ||                \
  ...
  if (r_->stride[0] == 1) {
#ifdef THC_REAL_IS_FLOAT
    THCudaBlas_Sger(state, vec1->size[0], vec2->size[0], alpha,
                    THCTensor_(data)(state, vec1), vec1->stride[0],
                    THCTensor_(data)(state, vec2), vec2->stride[0],
                    THCTensor_(data)(state, r_), r_->stride[1]);
#elif defined(THC_REAL_IS_DOUBLE)
    THCudaBlas_Dger(state, vec1->size[0], vec2->size[0], alpha,
                    THCTensor_(data)(state, vec1), vec1->stride[0],
                    THCTensor_(data)(state, vec2), vec2->stride[0],
                    THCTensor_(data)(state, r_), r_->stride[1]);
#elif defined(THC_REAL_IS_ZDOUBLE)
    THCudaBlas_Zger(state, vec1->size[0], vec2->size[0], alpha,
                    THCTensor_(data)(state, vec1), vec1->stride[0],
                    THCTensor_(data)(state, vec2), vec2->stride[0],
                    THCTensor_(data)(state, r_), r_->stride[1]);
#elif defined(THC_REAL_IS_ZFLOAT)
    THCudaBlas_Cger(state, vec1->size[0], vec2->size[0], alpha,
                    THCTensor_(data)(state, vec1), vec1->stride[0],
                    THCTensor_(data)(state, vec2), vec2->stride[0],
                    THCTensor_(data)(state, r_), r_->stride[1]);
#endif
...
#ifdef THC_REAL_IS_FLOAT
    THCudaBlas_Sger(state, vec2->size[0], vec1->size[0], alpha,
                    THCTensor_(data)(state, vec2), vec2->stride[0],
                    THCTensor_(data)(state, vec1), vec1->stride[0],
                    THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_DOUBLE)
    THCudaBlas_Dger(state, vec2->size[0], vec1->size[0], alpha,
                    THCTensor_(data)(state, vec2), vec2->stride[0],
                    THCTensor_(data)(state, vec1), vec1->stride[0],
                    THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_ZDOUBLE)
    THCudaBlas_Zger(state, vec2->size[0], vec1->size[0], alpha,
                    THCTensor_(data)(state, vec2), vec2->stride[0],
                    THCTensor_(data)(state, vec1), vec1->stride[0],
                    THCTensor_(data)(state, r_), r_->stride[0]);
#elif defined(THC_REAL_IS_ZFLOAT)
    THCudaBlas_Cger(state, vec2->size[0], vec1->size[0], alpha,
                    THCTensor_(data)(state, vec2), vec2->stride[0],
                    THCTensor_(data)(state, vec1), vec1->stride[0],
                    THCTensor_(data)(state, r_), r_->stride[0]);
#endif
...
THC_API void THCTensor_(addmm)(THCState *state, THCTensor *r_, real beta,
                               THCTensor *t, real alpha, THCTensor *m1,
                               THCTensor *m2) {
#if defined(THC_REAL_IS_HALF) || defined(THC_REAL_IS_FLOAT) ||                 \
    defined(THC_REAL_IS_DOUBLE) || defined(THC_REAL_IS_ZFLOAT) ||              \
    defined(THC_REAL_IS_ZDOUBLE)
  ...
  THCudaBlas_Hgemm(
      state, transpose_m1, transpose_m2,
      r__->size[(transpose_r == 'n' ? 0 : 1)],
      r__->size[(transpose_r == 'n' ? 1 : 0)],
      m1_->size[(transpose_r == 'n' ? 1 : 0)], alpha,
      ...
      beta, THCTensor_(data)(state, r__),
...
THC_API void THCTensor_(addbmm)(THCState *state, THCTensor *result, real beta,
                                THCTensor *t, real alpha, THCTensor *batch1,
                                THCTensor *batch2) {
#if defined(THC_REAL_IS_HALF) || defined(THC_REAL_IS_FLOAT) ||                 \
    defined(THC_REAL_IS_DOUBLE) || defined(THC_REAL_IS_ZFLOAT) ||              \
    defined(THC_REAL_IS_ZDOUBLE)
  ...
  THArgCheck(THCTensor_(nDimension)(state, batch1) == 3, 6,
             "expected 3D tensor");
  THArgCheck(THCTensor_(nDimension)(state, batch2) == 3, 7,
             "expected 3D tensor");
...
THC_API void THCTensor_(baddbmm)(THCState *state, THCTensor *result, real beta,
                                 THCTensor *t, real alpha, THCTensor *batch1,
                                 THCTensor *batch2) {
#if defined(THC_REAL_IS_HALF) || defined(THC_REAL_IS_FLOAT) ||                 \
    defined(THC_REAL_IS_DOUBLE) || defined(THC_REAL_IS_ZFLOAT) ||              \
  ...
  THArgCheck(THCTensor_(nDimension)(state, batch2) == 3, 7,
             "expected 3D tensor");
  THArgCheck(THCTensor_(size)(state, t, 0) ==
                 THCTensor_(size)(state, batch1, 0),
             6, "equal number of batches expected");
  THArgCheck(THCTensor_(size)(state, t, 0) ==
                 THCTensor_(size)(state, batch2, 0),
             7, "equal number of batches expected");
  THArgCheck(THCTensor_(size)(state, t, 1) ==
                 THCTensor_(size)(state, batch1, 1),
             6, "wrong matrix size");
  THArgCheck(THCTensor_(size)(state, t, 2) ==
                 THCTensor_(size)(state, batch2, 2),
             7, "wrong matrix size");
  THArgCheck(THCTensor_(size)(state, batch1, 2) ==
                 THCTensor_(size)(state, batch2, 1),
             6, "wrong matrix size");
...
#ifdef THC_REAL_IS_FLOAT
  THCudaBlas_SgemmBatched(state, transpose_batch1, transpose_batch2,
                          result_->size[transpose_result ? 2 : 1],
                          result_->size[transpose_result ? 1 : 2],
                          batch1_->size[transpose_result ? 1 : 2], alpha,
                          d_matrices1, lda, d_matrices2, ldb, beta,
                          d_result_matrices, ldc, num_batches);
#elif defined(THC_REAL_IS_DOUBLE)
  THCudaBlas_DgemmBatched(state, transpose_batch1, transpose_batch2,
                          result_->size[transpose_result ? 2 : 1],
                          result_->size[transpose_result ? 1 : 2],
                          batch1_->size[transpose_result ? 1 : 2], alpha,
                          d_matrices1, lda, d_matrices2, ldb, beta,
                          d_result_matrices, ldc, num_batches);
#elif defined(THC_REAL_IS_ZDOUBLE)
  THCudaBlas_ZgemmBatched(state, transpose_batch1, transpose_batch2,
                          result_->size[transpose_result ? 2 : 1],
                          result_->size[transpose_result ? 1 : 2],
                          batch1_->size[transpose_result ? 1 : 2], alpha,
                          d_matrices1, lda, d_matrices2, ldb, beta,
                          d_result_matrices, ldc, num_batches);
#elif defined(THC_REAL_IS_ZFLOAT)
  THCudaBlas_CgemmBatched(state, transpose_batch1, transpose_batch2,
                          result_->size[transpose_result ? 2 : 1],
                          result_->size[transpose_result ? 1 : 2],
                          batch1_->size[transpose_result ? 1 : 2], alpha,
                          d_matrices1, lda, d_matrices2, ldb, beta,
  ...
  for (long i = 0; i < num_batches; ++i) {
    THCudaBlas_Hgemm(
        ...
        THCTensor_(data)(state, batch2_) + i * batch2_->stride[0], ldb, beta,
...
THC_API void THCTensor_(btrifact)(THCState *state, THCTensor *ra_,
                                  THCudaIntTensor *rpivots_,
                                  THCudaIntTensor *rinfo_, int pivot,
                                  THCTensor *a) {
#if defined(THC_REAL_IS_FLOAT) || defined(THC_REAL_IS_DOUBLE) ||               \
    defined(THC_REAL_IS_ZFLOAT) || defined(THC_REAL_IS_ZDOUBLE)
  THAssert(THCTensor_(checkGPU)(state, 2, ra_, a));
  THArgCheck(THCTensor_(nDimension)(state, a) == 3, 3, "expected 3D tensor");
  THArgCheck(THCTensor_(size)(state, a, 1) == THCTensor_(size)(state, a, 2), 3,
             "matrices must be square");

  if (ra_ != a) {
    THCTensor_(resizeAs)(state, ra_, a);
    // not sure if this is kosher, but things are nicer if we return in column
    // major
    if (ra_->stride[0] == 1) {
  ...
  if (!pivot) {
    ...
    THCudaIntTensor **ptrs =
        (THCudaIntTensor **)THAlloc(sizeof(THCudaIntTensor *) * num_batches);
    for (long i = 0; i < num_batches; i++) {
      ptrs[i] = t;
    }
  ...
  if (rinfo_ == NULL)
    rinfo_ = THCudaIntTensor_new(state);
  ...
  const long grid = (num_batches + block - 1) / block;
  createBatchGemmBuffer<<<grid, block, 0, THCState_getCurrentStream(state)>>>(
      (const real **)d_result, THCTensor_(data)(state, ra__), ra__->stride[0],
      num_batches);
...
#if defined(THC_REAL_IS_FLOAT) || defined(THC_REAL_IS_DOUBLE)
    if (min != 0 || max != 0) {
      THError("failed to factorize some batch elements (min info == %d, max "
              "info == %d)",
              min, max);
#elif defined(THC_REAL_IS_ZDOUBLE)
    {
...
THC_API void THCTensor_(btrisolve)(THCState *state, THCTensor *rb_,
                                   THCTensor *b, THCTensor *atf,
                                   THCudaIntTensor *pivots) {
#if defined(THC_REAL_IS_FLOAT) || defined(THC_REAL_IS_DOUBLE) ||               \
    defined(THC_REAL_IS_ZFLOAT) || defined(THC_REAL_IS_ZDOUBLE)
  ...
  THArgCheck(THCTensor_(nDimension)(state, b) == 3 ||
                 THCTensor_(nDimension)(state, b) == 2,
             4, "expected 2D or 3D tensor");
  THArgCheck(THCTensor_(size)(state, atf, 0) == THCTensor_(size)(state, b, 0),
             3, "number of batches must be equal");
  THArgCheck(THCTensor_(size)(state, atf, 1) == THCTensor_(size)(state, atf, 2),
             3, "A matrices must be square");
  THArgCheck(THCTensor_(size)(state, atf, 1) == THCTensor_(size)(state, b, 1),
             3, "dimensions of A and b must be equal");
  ...
  const long grid = (num_batches + block - 1) / block;
  createBatchGemmBuffer<<<grid, block, 0, THCState_getCurrentStream(state)>>>(
      (const real **)d_result, THCTensor_(data)(state, rb__), rb__->stride[0],
      num_batches);
  createBatchGemmBuffer<<<grid, block, 0, THCState_getCurrentStream(state)>>>(
      d_atf, THCTensor_(data)(state, atf_), atf_->stride[0], num_batches);
  ...
  int *pivots_gpu;
  THCudaCheck(
      THCudaMalloc(state, (void **)&pivots_gpu, sizeof(int) * num_batches * n));
  THCudaIntTensor_resize1d(state, pivots, num_batches * n);
  THCudaCheck(cudaMemcpy(pivots_gpu, THCudaIntTensor_data(state, pivots),
                         sizeof(int) * num_batches * n,
...
#ifdef THC_REAL_IS_FLOAT
  THCudaBlas_Sgetrs(state, 'n', n, nrhs, d_atf, lda, pivots_gpu, d_result, ldb,
                    &info, num_batches);
#elif defined(THC_REAL_IS_DOUBLE)
  THCudaBlas_Dgetrs(state, 'n', n, nrhs, d_atf, lda, pivots_gpu, d_result, ldb,
                    &info, num_batches);
#elif defined(THC_REAL_IS_ZDOUBLE)
  THCudaBlas_Zgetrs(state, 'n', n, nrhs, d_atf, lda, pivots_gpu, d_result, ldb,
                    &info, num_batches);
#elif defined(THC_REAL_IS_ZFLOAT)
  THCudaBlas_Cgetrs(state, 'n', n, nrhs, d_atf, lda, pivots_gpu, d_result, ldb,
                    &info, num_batches);
#endif
