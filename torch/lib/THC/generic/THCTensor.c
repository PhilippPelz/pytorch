...
THCTensor *THCTensor_(newWithTensor)(THCState *state, THCTensor *tensor) {
  THCTensor *self = (THCTensor *)THAlloc(sizeof(THCTensor));
  THCTensor_(rawInit)(state, self);
  THCTensor_(setStorageNd)(state, self, tensor->storage, tensor->storageOffset,
                           tensor->nDimension, tensor->size, tensor->stride);
  return self;
}

/* Storage init */
THCTensor *THCTensor_(newWithStorage)(THCState *state, THCStorage *storage,
  ...
  THCTensor_(rawInit)(state, self);
  THCTensor_(setStorageNd)(state, self, storage, storageOffset,
...
THCTensor *THCTensor_(newWithStorage1d)(THCState *state, THCStorage *storage,
                                        ptrdiff_t storageOffset, int64_t size0,
                                        int64_t stride0) {
  return THCTensor_(newWithStorage4d)(state, storage, storageOffset, size0,
                                      stride0, -1, -1, -1, -1, -1, -1);
}

THCTensor *THCTensor_(newWithStorage2d)(THCState *state, THCStorage *storage,
                                        ptrdiff_t storageOffset, int64_t size0,
                                        int64_t stride0, int64_t size1,
                                        int64_t stride1) {
  return THCTensor_(newWithStorage4d)(state, storage, storageOffset, size0,
                                      stride0, size1, stride1, -1, -1, -1, -1);
}

THCTensor *THCTensor_(newWithStorage3d)(THCState *state, THCStorage *storage,
                                        ptrdiff_t storageOffset, int64_t size0,
                                        int64_t stride0, int64_t size1,
                                        int64_t stride1, int64_t size2,
                                        int64_t stride2) {
  return THCTensor_(newWithStorage4d)(state, storage, storageOffset, size0,
...
THCTensor *THCTensor_(newWithStorage4d)(THCState *state, THCStorage *storage,
                                        ptrdiff_t storageOffset, int64_t size0,
                                        int64_t stride0, int64_t size1,
                                        int64_t stride1, int64_t size2,
                                        int64_t stride2, int64_t size3,
                                        int64_t stride3) {
  int64_t size[4] = {size0, size1, size2, size3};
  int64_t stride[4] = {stride0, stride1, stride2, stride3};
}

THCTensor *THCTensor_(newWithSize)(THCState *state, THLongStorage *size,
                                   THLongStorage *stride) {
...
THCTensor *THCTensor_(newWithSize2d)(THCState *state, int64_t size0,
                                     int64_t size1) {
  return THCTensor_(newWithSize4d)(state, size0, size1, -1, -1);
}

THCTensor *THCTensor_(newWithSize3d)(THCState *state, int64_t size0,
                                     int64_t size1, int64_t size2) {
  return THCTensor_(newWithSize4d)(state, size0, size1, size2, -1);
}

THCTensor *THCTensor_(newWithSize4d)(THCState *state, int64_t size0,
                                     int64_t size1, int64_t size2,
                                     int64_t size3) {
  int64_t size[4] = {size0, size1, size2, size3};
...
THCTensor *THCTensor_(newSelect)(THCState *state, THCTensor *tensor,
...
THCTensor *THCTensor_(newNarrow)(THCState *state, THCTensor *tensor,
                                 int dimension_, int64_t firstIndex_,
                                 int64_t size_) {
  THCTensor *self = THCTensor_(newWithTensor)(state, tensor);
...
THCTensor *THCTensor_(newTranspose)(THCState *state, THCTensor *tensor,
...
THCTensor *THCTensor_(newUnfold)(THCState *state, THCTensor *tensor,
...
THCTensor *THCTensor_(newView)(THCState *state, THCTensor *tensor,
                               THLongStorage *size) {
  THArgCheck(THCTensor_(isContiguous)(state, tensor), 2,
             "input is not contiguous");
  ptrdiff_t numel = THCTensor_(nElement)(state, tensor);
  THCTensor *self = THCTensor_(new)(state);
  THLongStorage *inferred_size = THLongStorage_newInferSize(size, numel);
  THCTensor_(setStorage)(state, self, tensor->storage, tensor->storageOffset,
                         inferred_size, NULL);
  THLongStorage_free(inferred_size);
  return self;
}
THCTensor *THCTensor_(newExpand)(THCState *state, THCTensor *tensor,
                                 THLongStorage *sizes) {
...
/* Resize */
void THCTensor_(resize)(THCState *state, THCTensor *self, THLongStorage *size,
                        THLongStorage *stride) {
  THArgCheck(size != NULL, 2, "invalid size");
  if (stride)
    THArgCheck(stride->size == size->size, 3, "invalid stride");

  THCTensor_(resizeNd)(state, self, size->size, size->data,
                       (stride ? stride->data : NULL));
}
...
void THCTensor_(resize2d)(THCState *state, THCTensor *tensor, int64_t size0,
                          int64_t size1) {
  THCTensor_(resize4d)(state, tensor, size0, size1, -1, -1);
}

void THCTensor_(resize3d)(THCState *state, THCTensor *tensor, int64_t size0,
                          int64_t size1, int64_t size2) {
  THCTensor_(resize4d)(state, tensor, size0, size1, size2, -1);
}

void THCTensor_(resize4d)(THCState *state, THCTensor *self, int64_t size0,
                          int64_t size1, int64_t size2, int64_t size3) {
  int64_t size[4] = {size0, size1, size2, size3};

  THCTensor_(resizeNd)(state, self, 4, size, NULL);
}

void THCTensor_(resize5d)(THCState *state, THCTensor *self, int64_t size0,
                          int64_t size1, int64_t size2, int64_t size3,
                          int64_t size4) {
...
void THCTensor_(expand)(THCState *state, THCTensor *r, THCTensor *tensor,
                        THLongStorage *sizes) {
  THArgCheck(THCTensor_(nDimension)(state, tensor) > 0, 0,
             "can't expand an empty tensor");
  THArgCheck(THLongStorage_size(sizes) >= THCTensor_(nDimension)(state, tensor),
             1, "the number of sizes provided must be greater or equal to the "
                "number of dimensions in the tensor");

  int64_t *expandedSizes;
  int64_t *expandedStrides;
  char error_buffer[1024];
  int ret = THLongStorage_inferExpandGeometry(
      tensor->size, tensor->stride, THCTensor_(nDimension)(state, tensor),
      sizes, &expandedSizes, &expandedStrides, error_buffer, 1024);
  if (ret != 0) {
    THError(error_buffer);
    return;
  }
  THCTensor_(setStorageNd)(state, r, THCTensor_(storage)(state, tensor),
                           THCTensor_(storageOffset)(state, tensor),
                           THLongStorage_size(sizes), expandedSizes,
                           expandedStrides);
  THFree(expandedSizes);
  THFree(expandedStrides);
}

void THCTensor_(expandNd)(THCState *state, THCTensor **rets, THCTensor **ops,
                          int count) {
  for (int i = 0; i < count; ++i) {
    THArgCheck(THCTensor_(nDimension)(state, ops[i]) > 0, i,
               "can't expand empty tensor %d", i);
  }
  ...
  char error_buffer[1024];
  int ret = THLongStorage_inferSizeN(sizes, count, op_sizes, op_dims,
                                     error_buffer, 1024);
...
void THCTensor_(set)(THCState *state, THCTensor *self, THCTensor *src) {
  if (self != src)
    THCTensor_(setStorageNd)(state, self, src->storage, src->storageOffset,
                             src->nDimension, src->size, src->stride);
}

void THCTensor_(setStorage)(THCState *state, THCTensor *self,
  ...
  THCTensor_(setStorageNd)(
      state, self, storage_, storageOffset_,
      (size_ ? size_->size : (stride_ ? stride_->size : 0)),
      (size_ ? size_->data : NULL), (stride_ ? stride_->data : NULL));
}

void THCTensor_(setStorage1d)(THCState *state, THCTensor *self,
                              THCStorage *storage_, ptrdiff_t storageOffset_,
                              int64_t size0_, int64_t stride0_) {
  THCTensor_(setStorage4d)(state, self, storage_, storageOffset_, size0_,
                           stride0_, -1, -1, -1, -1, -1, -1);
}

void THCTensor_(setStorage2d)(THCState *state, THCTensor *self,
                              THCStorage *storage_, ptrdiff_t storageOffset_,
                              int64_t size0_, int64_t stride0_, int64_t size1_,
                              int64_t stride1_) {
  THCTensor_(setStorage4d)(state, self, storage_, storageOffset_, size0_,
                           stride0_, size1_, stride1_, -1, -1, -1, -1);
}

void THCTensor_(setStorage3d)(THCState *state, THCTensor *self,
                              THCStorage *storage_, ptrdiff_t storageOffset_,
                              int64_t size0_, int64_t stride0_, int64_t size1_,
                              int64_t stride1_, int64_t size2_,
                              int64_t stride2_) {
  THCTensor_(setStorage4d)(state, self, storage_, storageOffset_, size0_,
...
void THCTensor_(setStorage4d)(THCState *state, THCTensor *self,
                              THCStorage *storage_, ptrdiff_t storageOffset_,
                              int64_t size0_, int64_t stride0_, int64_t size1_,
                              int64_t stride1_, int64_t size2_,
                              int64_t stride2_, int64_t size3_,
                              int64_t stride3_) {
  int64_t size[4] = {size0_, size1_, size2_, size3_};
  int64_t stride[4] = {stride0_, stride1_, stride2_, stride3_};

  THCTensor_(setStorageNd)(state, self, storage_, storageOffset_, 4, size,
                           stride);
}
void THCTensor_(narrow)(THCState *state, THCTensor *self, THCTensor *src,
...
void THCTensor_(select)(THCState *state, THCTensor *self, THCTensor *src,
...
void THCTensor_(transpose)(THCState *state, THCTensor *self, THCTensor *src,
...
void THCTensor_(unfold)(THCState *state, THCTensor *self, THCTensor *src,
...
void THCTensor_(squeeze1d)(THCState *state, THCTensor *self, THCTensor *src,
                           int dimension) {
  int d;
...
void THCTensor_(unsqueeze1d)(THCState *state, THCTensor *self, THCTensor *src,
                             int dimension) {
  int d;
  ...
  self->size = (int64_t *)THRealloc(self->size,
                                    sizeof(int64_t) * (self->nDimension + 1));
  self->stride = (int64_t *)THRealloc(self->stride,
                                      sizeof(int64_t) * (self->nDimension + 1));
  self->stride =
      (long *)THRealloc(self->stride, sizeof(long) * (self->nDimension + 1));
  self->nDimension++;
...
int THCTensor_(isSize)(THCState *state, const THCTensor *self,
...
int THCTensor_(isSetTo)(THCState *state, const THCTensor *self,
                        const THCTensor *src) {
...
int THCTensor_(isSameSizeAs)(THCState *state, const THCTensor *self,
                             const THCTensor *src) {
  int d;
  if (self->nDimension != src->nDimension)
    return 0;
...
void THCTensor_(setStorageNd)(THCState *state, THCTensor *self,
                              THCStorage *storage, ptrdiff_t storageOffset,
                              int nDimension, int64_t *size, int64_t *stride) {
  /* storage */
...
void THCTensor_(resizeNd)(THCState *state, THCTensor *self, int nDimension,
  ...
  if (nDimension > 0) {
    if (nDimension != self->nDimension) {
      self->size =
          (int64_t *)THRealloc(self->size, sizeof(int64_t) * nDimension);
      self->stride =
          (int64_t *)THRealloc(self->stride, sizeof(int64_t) * nDimension);
    ...
    if (totalSize + self->storageOffset > 0) {
      ...
      if (totalSize + self->storageOffset > self->storage->size)
        THCStorage_(resize)(state, self->storage,
...
void THCTensor_(set1d)(THCState *state, THCTensor *tensor, int64_t x0,
                       real value) {
  THArgCheck(tensor->nDimension == 1, 1, "tensor must have one dimension");
  THArgCheck((x0 >= 0) && (x0 < tensor->size[0]), 2, "out of range");
  THCStorage_(set)(state, tensor->storage,
...
real THCTensor_(get1d)(THCState *state, const THCTensor *tensor, int64_t x0) {
  THArgCheck(tensor->nDimension == 1, 1, "tensor must have one dimension");
  THArgCheck((x0 >= 0) && (x0 < tensor->size[0]), 2, "out of range");
  return THCStorage_(get)(state, tensor->storage,
                          tensor->storageOffset + x0 * tensor->stride[0]);
}

void THCTensor_(set2d)(THCState *state, THCTensor *tensor, int64_t x0,
                       int64_t x1, real value) {
...
real THCTensor_(get2d)(THCState *state, const THCTensor *tensor, int64_t x0,
                       int64_t x1) {
  THArgCheck(tensor->nDimension == 2, 1, "tensor must have two dimensions");
  THArgCheck((x0 >= 0) && (x0 < tensor->size[0]) && (x1 >= 0) &&
                 (x1 < tensor->size[1]),
             2, "out of range");
  return THCStorage_(get)(state, tensor->storage, tensor->storageOffset +
                                                      x0 * tensor->stride[0] +
                                                      x1 * tensor->stride[1]);
}

void THCTensor_(set3d)(THCState *state, THCTensor *tensor, int64_t x0,
                       int64_t x1, int64_t x2, real value) {
...
real THCTensor_(get3d)(THCState *state, const THCTensor *tensor, int64_t x0,
...
void THCTensor_(set4d)(THCState *state, THCTensor *tensor, int64_t x0,
                       int64_t x1, int64_t x2, int64_t x3, real value) {
...
real THCTensor_(get4d)(THCState *state, const THCTensor *tensor, int64_t x0,
...
/* FIXME: remove this flag after any users stop using it since it is
  ...
  return 1;
#else
  int kernelP2PEnabled = THCState_getKernelPeerToPeerAccessEnabled(state);
