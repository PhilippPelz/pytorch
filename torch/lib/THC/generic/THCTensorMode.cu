...
THC_API void THCTensor_(calculateMode)(THCState *state, THCTensor *values,
                                       ...
                                       int dimension, THLongStorage *position) {
  ...
  for (int i = 0; i < THLongStorage_size(position); ++i) {
    data +=
        THLongStorage_data(position)[i] * THCTensor_(stride)(state, input, i);
  }

  int64_t nElement =
      THCTensor_(size)(state, input, THCTensor_(nDimension)(state, input) - 1);
  ...
  thrust::device_ptr<int64_t> sbPtr =
      thrust::device_pointer_cast(THCudaLongStorage_data(state, sortBuffer));
  ...
  int unique =
      ...
      thrust::inner_product(
          ...
          thrust::device,
#endif
          iter.begin(), iter.end() - 1, iter.begin() + 1, 0,
          thrust::plus<int>(),
#if defined(THC_REAL_IS_HALF)
  ...
  thrust::reduce_by_key(
      ...
      thrust::device,
#endif
      iter.begin(), iter.end(), thrust::constant_iterator<int>(1), keys.begin(),
      counts.begin()
#if defined(THC_REAL_IS_HALF)
          ,
      ThrustHalfEqualTo()
...
// Find first index within which it occurs
  ...
  THCStorage_(set)(state, THCTensor_(storage)(state, values), valuesOffset,
                   mode);
  THCudaLongStorage_set(state, THCudaLongTensor_storage(state, indices),
                        indicesOffset, index);
}

// this probably could be a loop, not a recursive algorithm
THC_API void THCTensor_(dimApplyMode)(THCState *state, THCTensor *values,
                                      THCudaLongTensor *indices,
                                      THCTensor *input,
                                      THCudaLongStorage *sortBuffer,
                                      int dimension, THLongStorage *position,
  ...
  if (curDim == ndim - 1) {
    THCTensor_(calculateMode)(state, values, indices, input, sortBuffer,
                              dimension, position);
  } else {
    // Loop through the values and recurse
    for (int i = 0; i < THCTensor_(size)(state, input, curDim); ++i) {
      position->data[curDim] = i;
      THCTensor_(dimApplyMode)(state, values, indices, input, sortBuffer,
                               dimension, position, curDim + 1);
    }
  }
}
...
THC_API void THCTensor_(mode)(THCState *state, THCTensor *values,
                              THCudaLongTensor *indices, THCTensor *input,
  ...
  if (sliceSize <= MAX_BLOCK_SIZE && slices <= MAX_GRID_SIZE &&
    ...
    // order to
    // properly determine the offset into the underlying storage in which to
    // place the
    // mode and index for a particular set of dimension values
    valuesTransposed =
        THCTensor_(newTranspose)(state, values, dimension, ndim - 1);
    indicesTransposed =
        THCudaLongTensor_newTranspose(state, indices, dimension, ndim - 1);

    // Set-up TensorInfo structs for passing to kernel
    TensorInfo<real, unsigned int> tiValues =
        getTensorInfo<THCTensor, unsigned int>(state, valuesTransposed);
    TensorInfo<int64_t, unsigned int> tiIndices =
        getTensorInfo<THCudaLongTensor, unsigned int>(state, indicesTransposed);

    // The number of blocks is the number of slices that we need to calculate
    // the mode for. Each block
...
#define HANDLE_MODE(SIZE)                                                      \
  {                                                                            \
    ...
    computeMode<                                                               \
        real,                                                                  \
        SIZE><<<grid, blockSize, memsize, THCState_getCurrentStream(state)>>>( \
    ...
    // Tensor, but
    // we need to be able to sort the inputs along the dimension in order to
    // calculate the
    // mode. Additionally, its ideal if the data along the dimension is
    // contiguous. So
    // we transpose the dimension with the innermost dimension and make a new
    // contiguous
    ...
    // Call mode
    THCTensor_(dimApplyMode)(state, valuesTransposed, indicesTransposed,
                             contiguous, sortBuffer, dimension, position, 0);
