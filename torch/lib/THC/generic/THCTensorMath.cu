...
#define HANDLE_CASE(DIMS)                                                      \
  CatArrayBatchedCopy<real, unsigned int,                                      \
                      DIMS><<<catGrid, applyBlock, 0, stream->stream>>>(       \
    ...
    for (i = 0; i < numInputs; i += CAT_ARRAY_BATCH_SIZE) {
      ...
      // hazard
      CatArrInputTensor<real, unsigned int> *stackInputs =
          (CatArrInputTensor<real, unsigned int> *)THCudaHostAlloc(
              state, tensorMetadataSize);
      cohortMax = 0;
      ...
      THCudaCheck(
          cudaMemcpyAsync(d_inputs, stackInputs,
...
void THCTensor_(diag)(THCState *state, THCTensor *self_, THCTensor *src_,
  ...
  if (nDimension == 2) {
    ...
    const dim3 threads(min(
        (int64_t)THCState_getCurrentDeviceProperties(state)->maxThreadsPerBlock,
        (int64_t)size));
    dim3 grid(
        min((int64_t)1024, (int64_t)THCCeilDiv(size, (int64_t)threads.x)));
    ...
    const dim3 threads(min(
        ...
        (int64_t)size));
    dim3 grid(
        min((int64_t)1024, (int64_t)THCCeilDiv(size, (ptrdiff_t)threads.x)));
...
void THCTensor_(arange)(THCState *state, THCTensor *r_, accreal xmin,
                        accreal xmax, accreal step) {
#if defined(THC_REAL_IS_ZFLOAT) || defined(THC_REAL_IS_ZDOUBLE)
  return THError("arange is not supported for complex type tensors");
#else
#if defined(THC_REAL_IS_FLOAT) || defined(THC_REAL_IS_DOUBLE) ||               \
    defined(THC_REAL_IS_HALF)
