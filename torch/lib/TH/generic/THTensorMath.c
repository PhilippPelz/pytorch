...
#ifdef _OPENMP
#define TH_TENSOR_APPLY3_CONTIG(TYPE1, TENSOR1, TYPE2, TENSOR2, TYPE3,         \
                                TENSOR3, CODE)                                 \
  {                                                                            \
    ptrdiff_t TH_TENSOR_size = THTensor_(nElement)(TENSOR1);                   \
    PRAGMA(omp parallel if (TH_TENSOR_size > TH_OMP_OVERHEAD_THRESHOLD)) {     \
...
void THTensor_(zero)(THTensor *r_) { THTensor_(fill)(r_, 0); }
...
void THTensor_(maskedCopy)(THTensor *tensor, THByteTensor *mask,
                           THTensor *src) {
  ...
  if (THTensor_(nElement)(tensor) != THByteTensor_nElement(mask)) {
    THTensor_(free)(srct);
    THError("Number of elements of destination tensor != Number of elements in "
            "mask");
  }
  TH_TENSOR_APPLY2(
...
void THTensor_(maskedSelect)(THTensor *tensor, THTensor *src,
                             THByteTensor *mask) {
...
#define IS_NONZERO(val) ((val) != 0)
  ...
  TH_TENSOR_APPLY(real, tensor, if IS_NONZERO(*tensor_data) { ++numel; });
#ifdef DEBUG
  THAssert(numel <= LONG_MAX);
#endif
  ...
  TH_TENSOR_APPLY(real, tensor, if IS_NONZERO(*tensor_data) {
    div = 1;
...
void THTensor_(indexSelect)(THTensor *tensor, THTensor *src, int dim,
                            THLongTensor *index) {
  ptrdiff_t i, numel;
  ...
  THArgCheck(dim < src->nDimension, 4,
             "Indexing dim %d is out of bounds of tensor", dim + TH_INDEX_BASE);
  ...
  if (dim == 0 && THTensor_(isContiguous)(src) &&
      THTensor_(isContiguous)(tensor)) {
    tensor_data = THTensor_(data)(tensor);
...
#pragma omp parallel for if (numel *rowsize >                                  \
                             TH_OMP_OVERHEAD_THRESHOLD) private(i)
      for (i = 0; i < numel; i++)
        memcpy(tensor_data + i * rowsize,
               src_data + (index_data[i] - TH_INDEX_BASE) * rowsize,
               rowsize * sizeof(real));
    }
  } else if (src->nDimension == 1) {
    for (i = 0; i < numel; i++)
      THTensor_(set1d)(tensor, i,
                       THTensor_(get1d)(src, index_data[i] - TH_INDEX_BASE));
  } else {
    for (i = 0; i < numel; i++) {
...
void THTensor_(indexCopy)(THTensor *tensor, int dim, THLongTensor *index,
                          THTensor *src) {
  ptrdiff_t i, numel;
  THTensor *tSlice, *sSlice;
  int64_t *index_data;
  ...
  THArgCheck(dim < src->nDimension, 4,
             "Indexing dim %d is out of bounds of tensor", dim + TH_INDEX_BASE);
  THArgCheck(numel == src->size[dim], 4,
  ...
  if (tensor->nDimension > 1) {
    ...
    for (i = 0; i < numel; i++) {
      THTensor_(set1d)(tensor, index_data[i] - TH_INDEX_BASE,
                       THTensor_(get1d)(src, i));
    }
  }
  THLongTensor_free(index);
}

void THTensor_(indexAdd)(THTensor *tensor, int dim, THLongTensor *index,
                         THTensor *src) {
  ptrdiff_t i, numel;
  THTensor *tSlice, *sSlice;
  int64_t *index_data;
  ...
  if (tensor->nDimension > 1) {
    ...
    for (i = 0; i < numel; i++) {
      THTensor_(set1d)(
          tensor, index_data[i] - TH_INDEX_BASE,
          THTensor_(get1d)(src, i) +
              THTensor_(get1d)(tensor, index_data[i] - TH_INDEX_BASE));
    }
  }
  THLongTensor_free(index);
}

void THTensor_(indexFill)(THTensor *tensor, int dim, THLongTensor *index,
                          real val) {
  ptrdiff_t i, numel;
  THTensor *tSlice;
  int64_t *index_data;
  ...
  numel = THLongTensor_nElement(index);
  THArgCheck(index->nDimension == 1, 3, "Index is supposed to be a vector");
  THArgCheck(dim < tensor->nDimension, 4,
             "Indexing dim %d is out of bounds of tensor", dim + TH_INDEX_BASE);
  ...
  TH_TENSOR_DIM_APPLY3(real, tensor, real, src, int64_t, index, dim,
                       for (i = 0; i < elems_per_row; ++i) {
                         ...
                         if (idx < TH_INDEX_BASE ||
                             idx >= src_size + TH_INDEX_BASE) {
                           THFree(TH_TENSOR_DIM_APPLY_counter);
                           THError("Invalid index in gather");
                         }
                         *(tensor_data + i * tensor_stride) =
                             src_data[(idx - TH_INDEX_BASE) * src_stride];
                       })
}

void THTensor_(scatter)(THTensor *tensor, int dim, THLongTensor *index,
                        THTensor *src) {
  int64_t elems_per_row, i, idx;
...
void THTensor_(scatterFill)(THTensor *tensor, int dim, THLongTensor *index,
  ...
  TH_TENSOR_DIM_APPLY2(
      real, tensor, int64_t, index, dim, for (i = 0; i < elems_per_row; ++i) {
        idx = *(index_data + i * index_stride);
...
real THTensor_(minall)(THTensor *tensor) {
  ...
  TH_TENSOR_APPLY(real, tensor, value = *tensor_data;
                  /* This is not the same as value<theMin in the case of NaNs */
                  if (!(CREAL(value) >= CREAL(theMin))) {
                    theMin = value;
                    th_isnan_break(CREAL(value))
                  });
  return theMin;
}
...
void THTensor_(add)(THTensor *r_, THTensor *t, real value) {
  ...
  if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t) &&
    ...
    TH_TENSOR_APPLY2_CONTIG(real, r_, real, t,
                            THVector_(adds)(r__data, t_data, value, r__len););
  } else {
...
#pragma omp parallel for if (sz > TH_OMP_OVERHEAD_THRESHOLD) private(i)
    for (i = 0; i < sz; i++)
      rp[i] = (tp[i] < min_value) ? min_value
                                  : (tp[i] > max_value ? max_value : tp[i]);
  } else {
    TH_TENSOR_APPLY2(real, r_, real, t,
                     *r__data =
                         (*t_data < min_value)
...
void THTensor_(cadd)(THTensor *r_, THTensor *t, real value, THTensor *src) {
  ...
  if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t) &&
    ...
    if (r_ == t) {
      THBlas_(axpy)(THTensor_(nElement)(t), value, THTensor_(data)(src), 1,
                    THTensor_(data)(r_), 1);
    } else {
...
#if defined(TH_REAL_IS_FLOAT)
    TH_TENSOR_APPLY3(real, r_, real, t, real, src,
                     *r__data = *t_data * powf(2, *src_data););
#elif defined(TH_REAL_IS_DOUBLE)
    TH_TENSOR_APPLY3(real, r_, real, t, real, src,
                     *r__data = *t_data * pow(2, *src_data););
#elif defined(TH_REAL_IS_BYTE)
    TH_TENSOR_APPLY3(real, r_, real, t, real, src, *r__data = ((real)*t_data)
                                                              << *src_data;);
#else
    TH_TENSOR_APPLY3(real, r_, real, t, real, src, *r__data = ((ureal)*t_data)
                                                              << *src_data;);
#endif // defined(TH_REAL_IS_FLOAT)
  }
...
void THTensor_(addcmul)(THTensor *r_, THTensor *t, real value, THTensor *src1,
                        THTensor *src2) {
  if (r_ != t) {
    THTensor_(resizeAs)(r_, t);
    THTensor_(copy)(r_, t);
  ...
  TH_TENSOR_APPLY3(real, r_, real, src1, real, src2,
                   *r__data += value * *src1_data * *src2_data;);
}

void THTensor_(addcdiv)(THTensor *r_, THTensor *t, real value, THTensor *src1,
                        THTensor *src2) {
  if (r_ != t) {
    THTensor_(resizeAs)(r_, t);
    THTensor_(copy)(r_, t);
...
void THTensor_(addmv)(THTensor *r_, real beta, THTensor *t, real alpha,
                      THTensor *mat, THTensor *vec) {
  if ((mat->nDimension != 2) || (vec->nDimension != 1))
    THError("matrix and vector expected, got %dD, %dD", mat->nDimension,
            vec->nDimension);
  ...
  if (mat->stride[0] == 1) {
    THBlas_(gemv)('n', mat->size[0], mat->size[1], alpha, THTensor_(data)(mat),
                  mat->stride[1], THTensor_(data)(vec), vec->stride[0], beta,
                  THTensor_(data)(r_), r_->stride[0]);
  } else if (mat->stride[1] == 1) {
    THBlas_(gemv)('t', mat->size[1], mat->size[0], alpha, THTensor_(data)(mat),
                  mat->stride[0], THTensor_(data)(vec), vec->stride[0], beta,
                  THTensor_(data)(r_), r_->stride[0]);
  } else {
    THTensor *cmat = THTensor_(newContiguous)(mat);

    THBlas_(gemv)('t', mat->size[1], mat->size[0], alpha, THTensor_(data)(cmat),
                  cmat->stride[0], THTensor_(data)(vec), vec->stride[0], beta,
                  THTensor_(data)(r_), r_->stride[0]);
...
void THTensor_(match)(THTensor *r_, THTensor *m1, THTensor *m2, real gain) {
  ...
  dim = m1->size[1];
  THArgCheck(m1->size[1] == m2->size[1], 3,
...
void THTensor_(addmm)(THTensor *r_, real beta, THTensor *t, real alpha,
                      THTensor *m1, THTensor *m2) {
  char transpose_r, transpose_m1, transpose_m2;
  THTensor *r__, *m1_, *m2_;

  if ((m1->nDimension != 2) || (m2->nDimension != 2))
    THError("matrices expected, got %dD, %dD tensors", m1->nDimension,
            m2->nDimension);
  ...
  THBlas_(gemm)(
      ...
      m1_->size[(transpose_r == 'n' ? 1 : 0)], alpha, THTensor_(data)(m1_),
      ...
      beta, THTensor_(data)(r__), r__->stride[(transpose_r == 'n' ? 1 : 0)]);
...
void THTensor_(addr)(THTensor *r_, real beta, THTensor *t, real alpha,
  ...
  if ((t->size[0] != vec1->size[0]) || (t->size[1] != vec2->size[0])) {
    ...
    THError("size mismatch, t: %s, vec1: %s, vec2: %s", bt.str, bv1.str,
            bv2.str);
  }
  ...
  if (r_->stride[0] == 1) {
    THBlas_(ger)(vec1->size[0], vec2->size[0], alpha, THTensor_(data)(vec1),
                 vec1->stride[0], THTensor_(data)(vec2), vec2->stride[0],
                 THTensor_(data)(r_), r_->stride[1]);
  } else if (r_->stride[1] == 1) {
    THBlas_(ger)(vec2->size[0], vec1->size[0], alpha, THTensor_(data)(vec2),
                 vec2->stride[0], THTensor_(data)(vec1), vec1->stride[0],
                 THTensor_(data)(r_), r_->stride[0]);
  } else {
...
void THTensor_(addbmm)(THTensor *result, real beta, THTensor *t, real alpha,
                       THTensor *batch1, THTensor *batch2) {
  int64_t batch;
  ...
  THArgCheck(THTensor_(size)(t, 0) == dim1, 1,
             "output tensor of incorrect size");
  THArgCheck(THTensor_(size)(t, 1) == dim2, 1,
             "output tensor of incorrect size");
...
void THTensor_(baddbmm)(THTensor *result, real beta, THTensor *t, real alpha,
                        THTensor *batch1, THTensor *batch2) {
  int64_t batch;

  THArgCheck(THTensor_(nDimension)(batch1) == 3, 1,
             "expected 3D tensor, got %dD", THTensor_(nDimension)(batch1));
  THArgCheck(THTensor_(nDimension)(batch2) == 3, 2,
             "expected 3D tensor, got %dD", THTensor_(nDimension)(batch2));
  THArgCheck(THTensor_(size)(batch1, 0) == THTensor_(size)(batch2, 0), 2,
             "equal number of batches expected, got %d, %d",
             THTensor_(size)(batch1, 0), THTensor_(size)(batch2, 0));
  THArgCheck(THTensor_(size)(batch1, 2) == THTensor_(size)(batch2, 1), 2,
  ...
  THArgCheck(THTensor_(size)(t, 1) == dim1, 1,
             "output tensor of incorrect size");
  THArgCheck(THTensor_(size)(t, 2) == dim2, 1,
             "output tensor of incorrect size");
  ...
  for (batch = 0; batch < THTensor_(size)(batch1, 0); ++batch) {
    ...
    THTensor_(addmm)(result_matrix, beta, result_matrix, alpha, matrix1,
                     matrix2);
  }
...
void THTensor_(max)(THTensor *values_, THLongTensor *indices_, THTensor *t,
  ...
  if (t->stride[dimension] == 1) {
    ...
    TH_TENSOR_DIM_APPLY3(
        real, t, real, values_, int64_t, indices_, dimension,
        theMax = t_data[0];
        theIndex = 0;
    ...
    TH_TENSOR_APPLY3_D(
        real, t, real, tempValues_, int64_t, tempIndices_, dimension,
...
void THTensor_(min)(THTensor *values_, THLongTensor *indices_, THTensor *t,
                    int dimension, int keepdim) {
...
void THTensor_(sum)(THTensor *r_, THTensor *t, int dimension, int keepdim) {
  ...
  // two implementations optimized for data locality
  if (t->stride[dimension] == 1) {
    TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension, accreal sum = 0;
                         int64_t i; for (i = 0; i < t_size; i++) sum +=
                                    t_data[i * t_stride];
                         *r__data = (real)sum;);
  } else {
    THTensor_(zero)(r_);
    ...
    TH_TENSOR_APPLY2(real, temp_, real, t,
...
void THTensor_(prod)(THTensor *r_, THTensor *t, int dimension, int keepdim) {
  ...
  // two implementations optimized for data locality
  if (t->stride[dimension] == 1) {
    TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension, accreal prod = 1; long i;
                         for (i = 0; i < t_size; i++) prod *=
                         t_data[i * t_stride];
                         *r__data = (real)prod;);
  } else {
    THTensor_(fill)(r_, 1);
...
void THTensor_(cumsum)(THTensor *r_, THTensor *t, int dimension) {
  ...
  TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension, accreal cumsum = 0;
                       int64_t i; for (i = 0; i < t_size; i++) {
...
void THTensor_(cumprod)(THTensor *r_, THTensor *t, int dimension) {
  ...
  TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension, accreal cumprod = 1;
                       int64_t i; for (i = 0; i < t_size; i++) {
...
void THTensor_(cross)(THTensor *r_, THTensor *a, THTensor *b, int dimension) {
  ...
  if (THTensor_(nDimension)(a) != THTensor_(nDimension)(b))
    THError("inconsistent tensor dimension %dD, %dD", THTensor_(nDimension)(a),
            THTensor_(nDimension)(b));
  ...
  THArgCheck(dimension >= 0 && dimension < THTensor_(nDimension)(a), 3,
             "dimension %d out of range", dimension + TH_INDEX_BASE);
  THArgCheck(THTensor_(size)(a, dimension) == 3, 3,
             "dimension %d does not have size 3", dimension + TH_INDEX_BASE);

  THTensor_(resizeAs)(r_, a);

  TH_TENSOR_DIM_APPLY3(
      real, a, real, b, real, r_, dimension,
      r__data[0 * r__stride] = a_data[1 * a_stride] * b_data[2 * b_stride] -
                               a_data[2 * a_stride] * b_data[1 * b_stride];
      r__data[1 * r__stride] = a_data[2 * a_stride] * b_data[0 * b_stride] -
                               a_data[0 * a_stride] * b_data[2 * b_stride];
      r__data[2 * r__stride] = a_data[0 * a_stride] * b_data[1 * b_stride] -
                               a_data[1 * a_stride] * b_data[0 * b_stride];);
}
...
void THTensor_(diag)(THTensor *r_, THTensor *t, int k) {
  THArgCheck(THTensor_(nDimension)(t) == 1 || THTensor_(nDimension)(t) == 2, 1,
...
static void THTensor_(quicksortascend)(real *arr, int64_t *idx,
                                       int64_t elements, int64_t stride) {
#if defined(TH_REAL_IS_ZFLOAT) || defined(TH_REAL_IS_ZDOUBLE)
...
static void THTensor_(quicksortdescend)(real *arr, int64_t *idx,
                                        int64_t elements, int64_t stride) {
#if defined(TH_REAL_IS_ZFLOAT) || defined(TH_REAL_IS_ZDOUBLE)
...
void THTensor_(sort)(THTensor *rt_, THLongTensor *ri_, THTensor *t,
  ...
  if (descendingOrder) {
    TH_TENSOR_DIM_APPLY2(
        real, rt_, int64_t, ri_, dimension, int64_t i;
...
static void THTensor_(quickselectnoidx)(real *arr, int64_t k, int64_t elements,
                                        int64_t stride) {
#if defined(TH_REAL_IS_ZFLOAT) || defined(TH_REAL_IS_ZDOUBLE)
  return THError("quickselectnoidx is not supported for complex type tensors");
#else
...
public domain implementation at http://ndevilla.free.fr/median/median/
Adapted similarly to the above Quicksort algorithm. */
static void THTensor_(quickselect)(real *arr, int64_t *idx, int64_t k,
                                   int64_t elements, int64_t stride) {
#if defined(TH_REAL_IS_ZFLOAT) || defined(TH_REAL_IS_ZDOUBLE)
...
void THTensor_(mode)(THTensor *values_, THLongTensor *indices_, THTensor *t,
  ...
  TH_TENSOR_DIM_APPLY3(
      real, t, real, values_, int64_t, indices_, dimension, int64_t i;
      real mode = 0; long modei = 0; long temp_freq = 0; long max_freq = 0;
...
void THTensor_(kthvalue)(THTensor *values_, THLongTensor *indices_, THTensor *t,
  ...
  THArgCheck(dimension >= 0 && dimension < THTensor_(nDimension)(t), 3,
             "dimension out of range");
  THArgCheck(k > 0 && k <= t->size[dimension], 2,
  ...
  TH_TENSOR_DIM_APPLY3(
      ...
      *values__data = temp__data[k - 1]; *indices__data = tempi__data[k - 1];);
...
void THTensor_(topk)(THTensor *rt_, THLongTensor *ri_, THTensor *t, int64_t k,
  ...
  if (dir) {
    ...
    TH_TENSOR_DIM_APPLY3(
        real, t, real, rt_, int64_t, ri_, dim, int64_t i;
    ...
    TH_TENSOR_DIM_APPLY3(
        ...
        if (sorted) THTensor_(quicksortascend)(tmp__data, tmpi__data, k - 1, 1);
        for (i = 0; i < k; i++) {
...
void THTensor_(catArray)(THTensor *result, THTensor **inputs, int numInputs,
                         int dimension) {
  THLongStorage *size;
  int i, j;
  int64_t offset;
  ...
  for (i = 0; i < maxDim; i++) {
    ...
    if (i == cat_dimension) {
      for (j = 1; j < numInputs; j++) {
        ...
        dimSize += i < inputs[j]->nDimension ? inputs[j]->size[i]
                                             : THMin(inputs[j]->nDimension, 1);
      }
    } else {
      for (j = 1; j < numInputs; j++) {
        int64_t sz =
            (i < inputs[j]->nDimension ? inputs[j]->size[i]
                                       : THMin(inputs[j]->nDimension, 1));
  ...
  if (!allEmpty) {
    ...
    if (cat_dimension == 0 && allContiguous) {
      ...
      for (j = 0; j < numInputs; j++) {
        if (inputs[j]->nDimension) {
          int64_t dimSize = cat_dimension < inputs[j]->nDimension
                                ? inputs[j]->size[cat_dimension]
                                : 1;
...
int THTensor_(equal)(THTensor *ta, THTensor *tb) {
  ...
  if (THTensor_(isContiguous)(ta) && THTensor_(isContiguous)(tb)) {
    ...
    TH_TENSOR_APPLY2(real, ta, real, tb, if (equal && *ta_data != *tb_data) {
      equal = 0;
...
#define LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(NAME, CFUNC)                       \
  void THTensor_(NAME)(THTensor * r_, THTensor * t) {                          \
    THTensor_(resizeAs)(r_, t);                                                \
    TH_TENSOR_APPLY2(real, r_, real, t, *r__data = CFUNC(*t_data););           \
  }
...
#define TH_MATH_NAME_REV(fn) f##fn
#define TH_MATH_NAME_CONST(fn) fn##f
#elif defined(TH_REAL_IS_ZFLOAT)
#define TH_MATH_NAME(fn) c##fn##f
#define TH_MATH_NAME_REV(fn) c##fn##f
#define TH_MATH_NAME_CONST(fn) fn
#elif defined(TH_REAL_IS_ZDOUBLE)
#define TH_MATH_NAME(fn) c##fn
#define TH_MATH_NAME_REV(fn) c##fn
#define TH_MATH_NAME_CONST(fn) fn
#else
#define TH_MATH_NAME(fn) fn
#define TH_MATH_NAME_REV(fn) fn
#define TH_MATH_NAME_CONST(fn) fn
#endif
...
void THTensor_(std)(THTensor *r_, THTensor *t, int dimension, int biased,
                    int keepdim) {
  THLongStorage *dim;
...
void THTensor_(var)(THTensor *r_, THTensor *t, int dimension, int biased,
                    int keepdim) {
  THLongStorage *dim;
...
void THTensor_(norm)(THTensor *r_, THTensor *t, real value, int dimension,
  ...
  if (value == 0) {
    TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension, accreal sum = 0;
                         int64_t i; for (i = 0; i < t_size; i++) sum += i;
                         *r__data = sum;)
  } else {
    TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension, accreal sum = 0;
                         int64_t i; for (i = 0; i < t_size; i++) {
...
#endif // defined(TH_REAL_IS_REAL)

#if defined(TH_REAL_IS_REAL) || defined(TH_REAL_IS_COMPLEX)

LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(log1p, log1p)
LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(sigmoid, TH_MATH_NAME(TH_sigmoid))
...
void THTensor_(pow)(THTensor *r_, THTensor *t, real value) {
  ...
  if (value == 1) {
    ...
    TH_TENSOR_APPLY2(real, r_, real, t,
                     *r__data = TH_MATH_NAME_CONST(1.0) / (*t_data * *t_data););
  } else {
    TH_TENSOR_APPLY2(real, r_, real, t,
                     *r__data = TH_MATH_NAME(pow)(*t_data, value););
  }
}
...
#endif // defined(TH_REAL_IS_REAL) || defined(TH_REAL_IS_COMPLEX)
...
LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(ceil, TH_CONCAT_2(Real, CEIL))
LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(floor, TH_CONCAT_2(Real, FLOOR))
LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(round, TH_CONCAT_2(Real, ROUND))
...
void THTensor_(log10)(THTensor *r_, THTensor *t) {
  THTensor_(resizeAs)(r_, t);
  if (THTensor_(isContiguous)(r_) && THTensor_(isContiguous)(t)) {
    TH_TENSOR_APPLY2_CONTIG(real, r_, real, t,
                            THVector_(log10)(r__data, t_data, r__len););
  } else {
    TH_TENSOR_APPLY2(real, r_, real, t, *r__data = TH_MATH_NAME(log)(*t_data) /
                                                   TH_MATH_NAME(log)(10););
  }
}
...
LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(rsqrt, TH_rsqrt)
LAB_IMPLEMENT_BASIC_FUNCTION(abs, TH_MATH_NAME_REV(abs))
LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(trunc, trunc)
LAB_IMPLEMENT_BASIC_FUNCTION_SIMPLE(frac, TH_MATH_NAME(TH_frac))
...
void THTensor_(renorm)(THTensor *res, THTensor *src, real value, int dimension,
  ...
  for (i = 0; i < src->size[dimension]; i++) {
    ...
    if (norm > maxnorm) {
      ...
      TH_TENSOR_APPLY2(real, rowR, real, rowS,
...
accreal THTensor_(varall)(THTensor *tensor, int biased) {
  ...
  TH_TENSOR_APPLY(real, tensor,
                  sum += (*tensor_data - mean) * (*tensor_data - mean););
...
void THTensor_(linspace)(THTensor *r_, real a, real b, int64_t n) {
  ...
  if (n == 1) {
    ...
    TH_TENSOR_APPLY(real, r_, *r__data = a + i * (b - a) / ((real)(n - 1));
                    i++;);
  }
}
...
void THTensor_(logspace)(THTensor *r_, real a, real b, int64_t n) {
  ...
  if (n == 1) {
#if defined(TH_REAL_IS_COMPLEX)
    return THError("logspace is not supported for complex type tensors");
#else
...
void THTensor_(histc)(THTensor *hist, THTensor *tensor, int64_t nbins,
  ...
  TH_TENSOR_APPLY(real, tensor, if (*tensor_data >= minval &&
                                    *tensor_data <= maxval) {
...
void THTensor_(bhistc)(THTensor *hist, THTensor *tensor, int64_t nbins,
                       real minvalue, real maxvalue) {
#if defined(TH_REAL_IS_ZFLOAT) || defined(TH_REAL_IS_ZDOUBLE)
  return THError("histc is not supported for complex type tensors");
#else
  ...
  int dimension = 1;
  THArgCheck(dimension >= 0 && dimension < THTensor_(nDimension)(tensor), 2,
             "invalid dimension %d", dimension + TH_INDEX_BASE);
  ...
  TH_TENSOR_DIM_APPLY2(real, tensor, real, hist, dimension, int64_t i;
                       for (i = 0; i < tensor_size; i++) {
                         if (tensor_data[i * tensor_stride] >= minval &&
                             tensor_data[i * tensor_stride] <= maxval) {
                           const int bin =
                               (int)((tensor_data[i * tensor_stride] - minval) /
                                     (maxval - minval) * nbins);
...
#undef TH_MATH_NAME
#undef TH_MATH_NAME_REV
#undef TH_MATH_NAME_CONST
#undef IS_NONZERO
#endif
