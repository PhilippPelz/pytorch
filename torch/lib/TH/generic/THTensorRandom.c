...
void THTensor_(clampedRandom)(THTensor *self, THGenerator *_generator, long min,
  ...
  TH_TENSOR_APPLY(
      real, self,
      *self_data = (real)((THRandom_random(_generator) % (max - min)) + min);)
}
...
void THTensor_(geometric)(THTensor *self, THGenerator *_generator, double p) {
  TH_TENSOR_APPLY(real, self,
                  *self_data = (real)THRandom_geometric(_generator, p););
}

void THTensor_(bernoulli)(THTensor *self, THGenerator *_generator, double p) {
  TH_TENSOR_APPLY(real, self,
                  *self_data = (real)THRandom_bernoulli(_generator, p););
}

void THTensor_(bernoulli_FloatTensor)(THTensor *self, THGenerator *_generator,
                                      THFloatTensor *p) {
  TH_TENSOR_APPLY2(real, self, float, p, *self_data = (real)THRandom_bernoulli(
                                             _generator, (double)*p_data););
}

void THTensor_(bernoulli_DoubleTensor)(THTensor *self, THGenerator *_generator,
                                       THDoubleTensor *p) {
  TH_TENSOR_APPLY2(real, self, double, p, *self_data = (real)THRandom_bernoulli(
                                              _generator, (double)*p_data););
}

#if defined(TH_REAL_IS_FLOAT) || defined(TH_REAL_IS_DOUBLE)

void THTensor_(uniform)(THTensor *self, THGenerator *_generator, double a,
                        double b) {
  TH_TENSOR_APPLY(real, self,
...
void THTensor_(normal)(THTensor *self, THGenerator *_generator, double mean,
                       double stdv) {
  TH_TENSOR_APPLY(real, self,
                  *self_data = (real)THRandom_normal(_generator, mean, stdv););
}

void THTensor_(normal_means)(THTensor *self, THGenerator *gen, THTensor *means,
                             double stddev) {
...
void THTensor_(normal_stddevs)(THTensor *self, THGenerator *gen, double mean,
                               THTensor *stddevs) {
...
void THTensor_(normal_means_stddevs)(THTensor *self, THGenerator *gen,
                                     THTensor *means, THTensor *stddevs) {
  THTensor_(resizeAs)(self, means);
  THTensor_(normal)(self, gen, 0, 1);
  THTensor_(cmul)(self, self, stddevs);
  THTensor_(cadd)(self, self, 1, means);
...
void THTensor_(cauchy)(THTensor *self, THGenerator *_generator, double median,
  ...
  TH_TENSOR_APPLY(real, self, *self_data = (real)THRandom_cauchy(
                                  _generator, median, sigma););
}

void THTensor_(logNormal)(THTensor *self, THGenerator *_generator, double mean,
                          double stdv) {
  TH_TENSOR_APPLY(real, self, *self_data = (real)THRandom_logNormal(
                                  _generator, mean, stdv););
}
...
void THTensor_(multinomialAliasSetup)(THTensor *probs, THLongTensor *j,
  ...
  THArgCheckWithCleanup((q_min > 0), THCleanup(THLongTensor_free(smaller);
                                               THLongTensor_free(larger);),
                        2, "q_min is less than 0");
...
void THTensor_(multinomial)(THLongTensor *self, THGenerator *_generator,
  ...
  for (i = 0; i < n_dist; i++) {
    ...
    for (j = 0; j < n_categories; j++) {
      sum += THStorage_(get)(prob_dist->storage, prob_dist->storageOffset +
                                                     i * prob_dist->stride[0] +
                                                     j * prob_dist->stride[1]);
      THDoubleStorage_set(cum_dist->storage,
                          cum_dist->storageOffset + j * cum_dist->stride[0],
                          sum);
    }
    THArgCheckWithCleanup(
        (sum > 0), THCleanup(THDoubleTensor_free(cum_dist);), 2,
    ...
    for (j = 0; j < n_sample; j++) {
      ...
      THDoubleTensor_data(cum_dist)[(n_categories - 1) * cum_dist->stride[0]] =
          1;

      while (right_pointer - left_pointer > 0) {
        mid_pointer = left_pointer + (right_pointer - left_pointer) / 2;
        cum_prob = THDoubleStorage_get(cum_dist->storage,
                                       cum_dist->storageOffset +
                                           mid_pointer * cum_dist->stride[0]);
      ...
      /* Once a sample is drawn, it cannot be drawn again. ie sample without
       * replacement */
      if (!with_replacement) {
        ...
        diff = THDoubleStorage_get(cum_dist->storage,
                                   cum_dist->storageOffset +
                                       sample_idx * cum_dist->stride[0]) -
               new_val;
        /* new sum of marginals is not one anymore... */
        sum = 1.0 - diff;
        for (k = 0; k < n_categories; k++) {
          new_val = THDoubleStorage_get(cum_dist->storage,
                                        cum_dist->storageOffset +
                                            k * cum_dist->stride[0]);
          if (k >= sample_idx) {
            /* remove sampled probability mass from later cumulative
             * probabilities */
            new_val -= diff;
          }
...
void THTensor_(getRNGState)(THGenerator *_generator, THTensor *self) {
  ...
  THArgCheck(THTensor_(isContiguous)(self), 1,
             "RNG state needs to be contiguous");
