...
void THTensor_(random)(THTensor *self, THGenerator *_generator) {
  ...
  TH_TENSOR_APPLY(real, self, *self_data =
                                  (unsigned char)(THRandom_random(_generator) %
                                                  (UCHAR_MAX + 1)););
  ...
  TH_TENSOR_APPLY(real,
                  self, *self_data = (double)(THRandom_random(_generator) %
                                              ((1ULL << DBL_MANT_DIG) + 1)););
#elif defined(TH_REAL_IS_ZFLOAT)
  TH_TENSOR_APPLY(
      real,
      self, *self_data = (float complex)(
...
void THTensor_(geometric)(THTensor *self, THGenerator *_generator, double p) {
  TH_TENSOR_APPLY(real,
                  self, *self_data = (real)THRandom_geometric(_generator, p););
}

void THTensor_(bernoulli)(THTensor *self, THGenerator *_generator, double p) {
  TH_TENSOR_APPLY(real,
                  self, *self_data = (real)THRandom_bernoulli(_generator, p););
}
...
void THTensor_(bernoulli_DoubleTensor)(THTensor *self, THGenerator *_generator,
  ...
  TH_TENSOR_APPLY2(real, self, double, p, *self_data = (real)THRandom_bernoulli(
    ...
    defined(TH_REAL_IS_ZFLOAT) || defined(TH_REAL_IS_ZDOUBLE)
...
void THTensor_(normal)(THTensor *self, THGenerator *_generator, double mean,
                       double stdv) {
  TH_TENSOR_APPLY(
      real, self, *self_data = (real)THRandom_normal(_generator, mean, stdv););
}

void THTensor_(exponential)(THTensor *self, THGenerator *_generator,
                            double lambda) {
  TH_TENSOR_APPLY(
      real, self, *self_data = (real)THRandom_exponential(_generator, lambda););
}
